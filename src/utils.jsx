...
function TabLoader() {
  return (
    <div style={{
      ...
      <div style={{fontSize:"0.75rem", color:"#6B6B6B", fontWeight:500}}>
        Loading module...
      </div>
...
const MOBILE_CSS = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* ── Hamburger ── */
  .pm-hamburger {
    display: none; background: transparent; border: 1px solid #E0E0E2;
  ...
  .pm-cam-aspect { position: relative; width: 100%; background: #EBEBED; border-radius: 14px; overflow: hidden; aspect-ratio: unset; height: 70vh; max-height: 70vh; }
  ...
  .pm-bnav-item-label { font-size: 0.92rem; font-weight: 600; color: #0D0D0D; flex: 1; }
  .pm-bnav-item.active .pm-bnav-item-label { color: #7c3aed; font-weight: 700; }
  .pm-bnav-item-pct { font-size: 0.6rem; font-weight: 700; color: #6B6B6B; background: #EBEBED; padding: 2px 7px; border-radius: 5px; }
  ...
  *, *::before, *::after {
  ...
  [data-theme="light"] .pm-bnav-item-label { color: #0D0D0D !important; }
  ...
  ::-webkit-scrollbar-thumb { background: #E0E0E2; border-radius: 4px; }
  ...
  .pm-region-chip {
    ...
    border: 1px solid #E0E0E2; background: transparent; color: #6B6B6B;
...
const THEMES = {
  dark: {
    ...
    card:"#ffffff",    inputBg:"#FFFFFF", inputBorder:"#E0E0E2",
...
/* ══════════════════════════════════════════════════════════
   REAL-TIME CLINICAL PHOTO UPLOAD — patient-specific images
   Captured/uploaded live during assessment (Special Tests, ROM,
   MMT, Kinetic Chain, Functional Screen, CPA, Neurological).
   Stored per-patient under data["img_"+itemId] as a JSON string
   array of {url,name,ts}. Backend is either Supabase Storage
   (bucket "clinical-photos", default) or Cloudinary (unsigned
   upload preset, configurable via the ⚙ gear on any widget).
   ══════════════════════════════════════════════════════════ */

if (typeof document !== "undefined" && !document.getElementById("pm-photo-style")) {
  const _pmStyle = document.createElement("style");
  _pmStyle.id = "pm-photo-style";
  _pmStyle.textContent = "@keyframes pmPhotoSpin { to { transform: rotate(360deg); } }";
  document.head.appendChild(_pmStyle);
}

function getImgBackendConfig() {
  try {
    return {
      backend: localStorage.getItem("pm_img_backend") || "supabase",
      cloud: localStorage.getItem("pm_img_cloud") || "",
      preset: localStorage.getItem("pm_img_preset") || "",
    };
  } catch { return { backend: "supabase", cloud: "", preset: "" }; }
}
function setImgBackendConfig({ backend, cloud, preset }) {
  try {
    if (backend !== undefined) localStorage.setItem("pm_img_backend", backend);
    if (cloud !== undefined) localStorage.setItem("pm_img_cloud", cloud);
    if (preset !== undefined) localStorage.setItem("pm_img_preset", preset);
  } catch {}
}

async function uploadClinicalPhoto(file, folderPath) {
  const cfg = getImgBackendConfig();
  if (cfg.backend === "cloudinary") {
    if (!cfg.cloud || !cfg.preset) {
      throw new Error("Cloudinary not configured — tap ⚙ to set cloud name & unsigned upload preset.");
    }
    const fd = new FormData();
    fd.append("file", file);
    fd.append("upload_preset", cfg.preset);
    fd.append("folder", "physiomind_patients/" + folderPath);
    const r = await fetch(`https://api.cloudinary.com/v1_1/${cfg.cloud}/image/upload`, { method: "POST", body: fd });
  ...
  const key = `${folderPath}/${Date.now()}_${Math.random().toString(36).slice(2,8)}.${ext}`;
  const { error } = await supabase.storage.from("clinical-photos").upload(key, file, { upsert: false, cacheControl: "3600" });
  if (error) {
    const notFound = /bucket not found/i.test(error.message || "");
    throw new Error(notFound
      ? "Storage bucket 'clinical-photos' not found — create a public bucket named exactly \"clinical-photos\" in your Supabase project (Storage → New bucket), or switch to Cloudinary via ⚙."
      : (error.message || "Supabase upload failed"));
  }
  const { data: pub } = supabase.storage.from("clinical-photos").getPublicUrl(key);
  return pub.publicUrl;
}

const _pmLbBtn = { padding:"6px 12px", borderRadius:99, border:"none", background:"rgba(255,255,255,0.15)", color:"#fff", fontSize:"0.7rem", fontWeight:700, cursor:"pointer" };

function ImgBackendSettingsModal({ onClose }) {
  const cfg0 = getImgBackendConfig();
  const [backend, setBackend] = useState(cfg0.backend);
  const [cloud, setCloud] = useState(cfg0.cloud);
  const [preset, setPreset] = useState(cfg0.preset);
  const C = getC();
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:100000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:20,maxWidth:340,width:"100%"}}>
        <div style={{fontWeight:800,fontSize:"0.9rem",color:C.text,marginBottom:12}}>📷 Clinical Photo Storage</div>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {["supabase","cloudinary"].map(b=>(
            <button key={b} type="button" onClick={()=>setBackend(b)}
              style={{flex:1,padding:"8px 6px",borderRadius:8,fontSize:"0.72rem",fontWeight:700,cursor:"pointer",
                border:`1.5px solid ${backend===b?C.accent:C.border}`,background:backend===b?`${C.accent}18`:"transparent",color:backend===b?C.accent:C.muted}}>
              {b==="supabase"?"Supabase Storage":"Cloudinary"}
            </button>
          ))}
        </div>
        {backend==="cloudinary" && (
          <>
            <input value={cloud} onChange={e=>setCloud(e.target.value)} placeholder="Cloud name (e.g. dr15y1pwj)"
              style={{width:"100%",boxSizing:"border-box",padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,background:C.s2,color:C.text,fontSize:"0.78rem",marginBottom:8}}/>
            <input value={preset} onChange={e=>setPreset(e.target.value)} placeholder="Unsigned upload preset"
              style={{width:"100%",boxSizing:"border-box",padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,background:C.s2,color:C.text,fontSize:"0.78rem",marginBottom:8}}/>
          </>
        )}
        {backend==="supabase" && (
          <div style={{fontSize:"0.72rem",color:C.muted,lineHeight:1.6,marginBottom:8}}>
            Uploads go to a Supabase Storage bucket named <strong>clinical-photos</strong>. Create it (public) in your Supabase project if it doesn't exist yet.
          </div>
        )}
        <div style={{display:"flex",gap:8,marginTop:8}}>
          <button type="button" onClick={onClose} style={{flex:1,padding:"9px",borderRadius:8,border:`1.5px solid ${C.border}`,background:"transparent",color:C.muted,fontSize:"0.78rem",fontWeight:700,cursor:"pointer"}}>Cancel</button>
          <button type="button" onClick={()=>{ setImgBackendConfig({backend,cloud,preset}); onClose(); }}
            style={{flex:1,padding:"9px",borderRadius:8,border:"none",background:C.accent,color:"#fff",fontSize:"0.78rem",fontWeight:700,cursor:"pointer"}}>Save</button>
        </div>
      </div>
    </div>
  );
...
// PatientPhotoUpload — real-time patient photo capture/upload for a specific
// clinical item (test/movement/muscle/level). Drop-in next to any existing
// ClinicalImage/ClinicalImageCard/SmallClinicalImg reference-image widget.
function PatientPhotoUpload({ itemId, label, data, set, size=44 }) {
  const C = getC();
  ...
  let photos = [];
  try { photos = data && data[key] ? JSON.parse(data[key]) : []; } catch { photos = []; }

  const persist = (arr) => set(key, JSON.stringify(arr));
  ...
  const handleFiles = async (e) => {
    ...
    for (const f of files) {
      try {
        const url = await uploadClinicalPhoto(f, itemId);
        arr = [...arr, { url, name: f.name, ts: Date.now() }];
        persist(arr);
      } catch (err) {
  ...
  const delPhoto = (idx) => {
    ...
    persist(arr);
    if (arr.length === 0) setLbOpen(false);
    else setLbIdx(Math.min(lbIdx, arr.length - 1));
  };

  const latest = photos[photos.length - 1];

  return (
    <div style={{ position:"relative", flexShrink:0, display:"inline-block" }} onClick={e=>e.stopPropagation()}>
      <div
        onClick={() => photos.length ? setLbOpen(true) : fileRef.current?.click()}
        title={photos.length ? `${photos.length} photo(s) — tap to view` : "Tap to add a live photo"}
        style={{
          width:size, height:size, borderRadius:8, cursor:"pointer", overflow:"hidden",
          border: photos.length ? `2px solid ${C.green}70` : `1.5px dashed ${C.border}`,
          background: photos.length ? C.s2 : C.s3,
          display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, position:"relative"
        }}>
        {busy ? (
          <div style={{width:14,height:14,border:`2px solid ${C.border}`,borderTopColor:C.accent,borderRadius:"50%",animation:"pmPhotoSpin 0.7s linear infinite"}}/>
        ) : latest ? (
          <>
            <img src={latest.url} alt={label} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            {photos.length>1 && <span style={{position:"absolute",bottom:1,right:1,background:"rgba(0,0,0,0.65)",color:"#fff",fontSize:8,fontWeight:800,padding:"0 3px",borderRadius:4}}>+{photos.length-1}</span>}
          </>
        ) : (
          <span style={{fontSize: size>32 ? "1.1rem" : "0.8rem", opacity:0.5}}>📷</span>
        )}
      </div>
      <button type="button" onClick={()=>setCfgOpen(true)} title="Photo storage settings"
        style={{position:"absolute",top:-6,right:-6,width:16,height:16,borderRadius:"50%",border:`1px solid ${C.border}`,background:C.surface,color:C.muted,fontSize:8,lineHeight:"14px",padding:0,cursor:"pointer"}}>⚙</button>
      <input ref={fileRef} type="file" accept="image/*" capture="environment" multiple
        style={{display:"none"}} onChange={handleFiles}/>
      {cfgOpen && <ImgBackendSettingsModal onClose={()=>setCfgOpen(false)}/>}
      {lbOpen && photos.length>0 && (
        <div onClick={()=>setLbOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:99999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,maxWidth:"92vw"}}>
            <div style={{color:"#fff",fontWeight:700,fontSize:"0.85rem"}}>{label}</div>
            <img src={photos[lbIdx]?.url} alt={label} style={{maxWidth:"88vw",maxHeight:"66vh",borderRadius:10,objectFit:"contain"}}/>
            <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
              {photos.length>1 && <button type="button" onClick={()=>setLbIdx(i=>(i-1+photos.length)%photos.length)} style={_pmLbBtn}>← Prev</button>}
              <span style={{color:"rgba(255,255,255,0.7)",fontSize:"0.72rem"}}>{lbIdx+1} / {photos.length}</span>
              {photos.length>1 && <button type="button" onClick={()=>setLbIdx(i=>(i+1)%photos.length)} style={_pmLbBtn}>Next →</button>}
              <button type="button" onClick={()=>fileRef.current?.click()} style={{..._pmLbBtn,background:"rgba(0,201,122,0.3)"}}>+ Add</button>
              <button type="button" onClick={()=>delPhoto(lbIdx)} style={{..._pmLbBtn,background:"rgba(255,77,109,0.3)"}}>🗑 Delete</button>
              <button type="button" onClick={()=>setLbOpen(false)} style={_pmLbBtn}>✕ Close</button>
            </div>
          </div>
        </div>
      )}
...
export { PatientPhotoUpload, uploadClinicalPhoto, getImgBackendConfig, setImgBackendConfig };
