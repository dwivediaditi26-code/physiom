  ...
  const regionResults = selectedRegions.map(region => {
    ...
    if (region === "Knee (L)" || region === "Knee (R)") {
      ...
      objTests = urgentFlag ? ["Defer loading — urgent referral as above"] :
        ["Gait observation — antalgic / varus thrust","Knee AROM — extension and flexion","Valgus/varus stress 0° + 30°",
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14, maxWidth:"100%" }}>
        ...
        <>
          ...
          <button type="button" onClick={runInterpretation}
            ...
            style={{
              ...
              fontWeight:800, fontSize:"0.88rem", cursor: selectedRegions.length > 0 ? "pointer" : "not-allowed",
      ...
      {activeTab === "results" && insight && showInsight && (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          {/* Per-region results */}
          {insight.regionResults.map((r, i) => (
            <div key={i} style={{
              background: r.urgentFlag ? "#fff5f5" : PC.surface,
              borderRadius:12, padding:"16px 18px",
              border:`2px solid ${r.urgentFlag ? PC.red : (RC_S[r.region]||PC.accent)}`,
              boxShadow:`0 2px 8px ${r.urgentFlag ? PC.red+"22" : PC.border}`,
            }}>
              {/* Region label */}
              <div style={{ fontSize:"0.6rem", fontWeight:700, textTransform:"uppercase",
                letterSpacing:1.2, color: RC_S[r.region]||PC.accent, marginBottom:4 }}>
                Region {i+1} — {r.region}
              </div>

              {/* Primary pattern */}
              <div style={{ fontSize:"0.92rem", fontWeight:800,
                color: r.urgentFlag ? PC.red : PC.text, marginBottom:8, lineHeight:1.3 }}>
                {r.primaryPattern}
              </div>

              {/* Confidence + tags row */}
              <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:8, alignItems:"center" }}>
                <span style={{ fontSize:"0.6rem", fontWeight:800, padding:"3px 10px", borderRadius:99,
                  background: confBg(r.confidence), color: confColor(r.confidence),
                  border:`1px solid ${confColor(r.confidence)}44` }}>
                  {r.confidence} CONFIDENCE
                </span>
                {r.tags.map(t => (
                  <span key={t} style={{ fontSize:"0.6rem", fontWeight:700, padding:"2px 8px",
                    borderRadius:99, whiteSpace:"nowrap",
                    background: t.includes("⚠") ? PC.red+"12" : PC.s3,
                    color: t.includes("⚠") ? PC.red : PC.muted,
                    border:`1px solid ${t.includes("⚠") ? PC.red+"33" : PC.border}` }}>
                    {t}
                  </span>
                ))}
              </div>
              ...
              {/* Precautions */}
              {r.precautions && r.precautions.filter(p=>p).length > 0 && (
                <div style={{ marginBottom:12 }}>
                  {r.precautions.filter(p=>p).map((p, j) => (
                    <div key={j} style={{
                      background: PC.red+"08", borderRadius:8, padding:"8px 12px",
                      fontSize:"0.72rem", color: PC.red, marginBottom:4,
                      borderLeft:`3px solid ${PC.red}`, lineHeight:1.5,
                    }}>{p}</div>
                  ))}
                </div>
              )}
              ...
              {r.differentials && r.differentials.length > 0 && (
                <div style={{ marginBottom:12 }}>
                  ...
                  {r.differentials.map((d, j) => (
                    <div key={j} style={{
                      background: j===0 ? (PC.accent+"08") : PC.s2,
                      border:`1px solid ${j===0 ? PC.accent+"33" : PC.border}`,
                      borderRadius:10, padding:"10px 14px", marginBottom:6,
                    }}>
                      <div style={{ display:"flex", justifyContent:"space-between",
                        alignItems:"flex-start", marginBottom:4 }}>
                        <div style={{ fontSize: j===0 ? "0.8rem":"0.74rem",
                          fontWeight: j===0 ? 800 : 600,
                          color: j===0 ? PC.text : PC.muted, flex:1, paddingRight:8 }}>
                          {j===0 ? "① " : j===1 ? "② " : "③ "}{d.label}
                        </div>
                        <span style={{ fontSize:"0.58rem", fontWeight:700, padding:"2px 7px",
                          borderRadius:99, whiteSpace:"nowrap",
                          background: confBg(d.confidence), color: confColor(d.confidence),
                          border:`1px solid ${confColor(d.confidence)}44` }}>
                          {d.confidence}
                        </span>
                      </div>
                      {d.evidence && (
                        <div style={{ fontSize:"0.65rem", color: PC.muted,
                          marginBottom: j===0 && d.tests?.length ? 8 : 0, lineHeight:1.45 }}>
                          Evidence: {d.evidence}
                        </div>
                      )}
                      {j === 0 && d.tests && d.tests.length > 0 && (
                        <div style={{ borderTop:`1px solid ${PC.border}`, paddingTop:8, marginTop:6 }}>
                          <div style={{ fontSize:"0.6rem", fontWeight:700, textTransform:"uppercase",
                            letterSpacing:0.8, color: PC.green, marginBottom:6 }}>
                            Recommended objective tests:
                          </div>
                          {d.tests.map((t, k) => (
                            <div key={k} style={{ fontSize:"0.68rem", color: PC.text,
                              marginBottom:4, paddingLeft:10,
                              borderLeft:`2px solid ${PC.green}44`, lineHeight:1.4 }}>
                              {t}
                            </div>
                          ))}
                        </div>
              ...
              {r.objTests && r.objTests.filter(t=>t).length > 0 && (
                <div style={{ background: PC.green+"08", border:`1px solid ${PC.green}33`,
                  borderRadius:10, padding:"10px 14px" }}>
                  <div style={{ fontSize:"0.62rem", fontWeight:700, textTransform:"uppercase",
                    letterSpacing:1, color: PC.green, marginBottom:8 }}>
                    🔍 Complete Objective Assessment Focus
                  </div>
                  {r.objTests.filter(t=>t).map((t, k) => (
                    <div key={k} style={{ fontSize:"0.7rem", color: PC.text,
                      marginBottom:5, paddingLeft:10,
                      borderLeft:`2px solid ${PC.green}55`, lineHeight:1.45 }}>
                      {t}
                    </div>
                  ))}
                </div>
          ...
          {insight.cross && insight.cross.length > 0 && (
            <div style={{ background: PC.surface, borderRadius:12, padding:"14px 16px",
              ...
              <div style={{ fontSize:"0.65rem", fontWeight:700, textTransform:"uppercase",
                ...
                🔗 Cross-Region Clinical Interactions — {insight.cross.length}
              </div>
          ...
          <div style={{ background: PC.green+"08", border:`1px solid ${PC.green}33`,
            ...
            <div style={{ fontSize:"0.65rem", fontWeight:700, textTransform:"uppercase",
              letterSpacing:1, color: PC.green, marginBottom:6 }}>
              Assessment Summary
            </div>
            <div style={{ fontSize:"0.74rem", color: PC.text, lineHeight:1.65 }}>
              {insight.regionResults.length} region{insight.regionResults.length>1?"s":""} assessed.{" "}
              {insight.anyUrgent && (
                <span style={{ color: PC.red, fontWeight:700 }}>
                  ⚠ Urgent flags present — do not defer medical review.{" "}
                </span>
              )}
              {insight.cross && insight.cross.length > 0 &&
                `${insight.cross.length} cross-region interaction${insight.cross.length>1?"s":""} identified. `}
              {selectedRegions.length >= 3 &&
                <span style={{ color: PC.yellow }}>3-region presentation — nociplastic screening recommended.</span>}
            </div>
            <div style={{ fontSize:"0.58rem", color: PC.muted, marginTop:8,
              borderTop:`1px solid ${PC.green}22`, paddingTop:6, fontStyle:"italic" }}>
              Engine v6 · Ranked differentials · Confidence scoring · NRS integration
              · Evidence: Magee(7th) · Petty(5th) · Maitland(8th) · Jones(2nd) · Sahrmann · Butler · McKenzie · Brukner & Khan(5th) · NICE NG59 · ASAS
            </div>
          </div>
        </div>
      )}
...
export { SpecialTestsSection, SubjectiveModule, NKTSection, KineticChainSection, FMASection, FasciaSection, NKT_REGIONS, KC_REGIONS, UNIV_S, REG_MOD_S, BPS_S, SLEEP_S, SPORT_S, runEngineV6, ErgoModule, CyriaxModule, CyriaxRegionTests, CYRIAX_REGIONS_DATA, generateDiagnosis, PDF_BASE_STYLES, makePDFPage, MOVEMENTS };
