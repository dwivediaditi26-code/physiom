...
import { ChevronLeft, Send, MapPin, IndianRupee, BadgeCheck } from "lucide-react";
  ...
  const [thread, setThread] = useState(() => [
    {
      id: "m1",
      isSelf: true,
      text: `Hi ${opp.mentor.name.split(",")[0]}, I'm very interested in this ${opp.title}! I've attached my verified profile below. My background is strong in ${opp.tags?.[0] || "this area"}.`,
    },
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
        {thread.map((m) => (
          <div key={m.id} className={`flex ${m.isSelf ? "justify-end" : "justify-start"}`}>
            {m.attachProfile ? (
              <div className="max-w-[80%] bg-slate-100 rounded-2xl px-3 py-2.5 flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10.5px] text-slate-500 truncate">Verified PhysioFeed profile</p>
                </div>
              </div>
            ) : (
