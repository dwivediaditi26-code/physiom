...
// ── SOAPDocViewer ────────────────────────────────────────────────────────────
// Inline version of the Live SOAP panel — same renderer, same data source,
// rendered as a normal page section (not floating). Used by the SOAP docs tab.
// ─────────────────────────────────────────────────────────────────────────────
function SOAPDocViewer({ data, onNavigate }) {
  const [tab, setTab] = React.useState("S");
  const [copied, setCopied] = React.useState(null);

  const soap = React.useMemo(() => buildRealtimeSOAP(data), [data]);
  ...
  const sectionCounts = React.useMemo(() => {
    if (!data) return { S:0, O:0, A:0, P:0 };
    const allKeys = Object.keys(data).filter(k => data[k] !== "" && data[k] !== null && data[k] !== undefined);
    const vk = k => !!data[k];
    const subjectiveKeys = ["cc_main","cc_vas_now","cc_quality","dem_name","cc_duration","cc_onset"];
    const sCount = subjectiveKeys.filter(vk).length;
    const oCount = Math.min(allKeys.filter(k =>
      k.startsWith("rom_") || k.startsWith("mmt_") || k.startsWith("st_") ||
      k.startsWith("gait_") || k.startsWith("post_") || k.startsWith("palp_") || k.startsWith("om_")
    ).length, 20);
    const aCount = [data.soap_a_diagnosis, data.soap_icd10, data.soap_a].filter(Boolean).length;
    const pCount = [data.soap_modalities, data.soap_frequency, data.hep_programme, data.tx_techniques].filter(Boolean).length;
    return { S:sCount, O:oCount, A:aCount, P:pCount };
  }, [data]);
  ...
  const currentSec = SECTION_LABELS2[tab];
  const currentText = soap?.[tab] || "";

  const formatText = (text) => {
    if (!text) return <div style={{color:"#9ca3af",fontSize:"0.82rem",textAlign:"center",padding:"24px 0"}}>No data yet — complete the {currentSec.label} section first.</div>;
    return text.split("\n").map((line, i) => {
      const trimmed = line.trim();
      if (!trimmed) return <div key={i} style={{height:6}}/>;
      const isHeader = /^[A-Z][A-Za-z \/()]+:$/.test(trimmed);
      const isIndented = line.startsWith("  ");
      return (
        <div key={i} style={{
          fontSize: isHeader ? "0.72rem" : "0.82rem",
          fontWeight: isHeader ? 800 : isIndented ? 400 : 500,
          color: isHeader ? currentSec.col : "#374151",
          textTransform: isHeader ? "uppercase" : "none",
          letterSpacing: isHeader ? "0.05em" : 0,
          paddingLeft: isIndented ? 14 : 0,
          borderLeft: isIndented ? `2px solid ${currentSec.col}30` : "none",
          marginBottom: isHeader ? 4 : 1,
          lineHeight: 1.55,
        }}>{trimmed}</div>
      );
    });
  ...
  return (
    <div style={{background:"#fff",borderRadius:14,border:"1px solid rgba(124,58,237,0.15)",overflow:"hidden",boxShadow:"0 2px 16px rgba(124,58,237,0.08)"}}>
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#7c3aed,#9333ea)",padding:"12px 16px",display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:"1rem"}}>📋</span>
        <span style={{flex:1,fontSize:"0.82rem",fontWeight:800,color:"#fff",letterSpacing:"0.3px"}}>SOAP Documentation</span>
        <div style={{display:"flex",alignItems:"center",gap:4}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:"#4ade80",boxShadow:"0 0 6px #4ade80"}}/>
          <span style={{fontSize:"0.72rem",color:"rgba(255,255,255,0.85)",fontWeight:700}}>LIVE</span>
        </div>
      </div>
      {/* Tabs */}
      <div style={{display:"flex",borderBottom:"1px solid rgba(124,58,237,0.12)",background:"#faf5ff"}}>
        {Object.entries(SECTION_LABELS2).map(([key,sec]) => {
          const active = tab === key;
          ...
          return (
            <button key={key} onClick={()=>setTab(key)} style={{
              ...
              <span style={{fontSize:"0.82rem",fontWeight:active?800:600,color:active?sec.col:"#7e6a9a"}}>{key}</span>
              {count > 0 && (
                <span style={{fontSize:"0.55rem",fontWeight:800,background:sec.col+"15",color:sec.col,padding:"0px 5px",borderRadius:8}}>{count}</span>
              )}
            </button>
          );
        })}
      ...
      <div style={{padding:"14px 16px",minHeight:200}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:"0.9rem"}}>{currentSec.icon}</span>
            <span style={{fontSize:"0.82rem",fontWeight:800,color:currentSec.col,letterSpacing:"0.5px",textTransform:"uppercase"}}>{currentSec.label}</span>
          </div>
          <button onClick={()=>copySection(currentText)} style={{
        ...
        <div style={{lineHeight:1.6}}>{formatText(currentText)}</div>
      </div>
    </div>
  );
}
...
export { GaitModule, OutcomeMeasuresModule, buildClinicalInterpretation, buildRealtimeSOAP,
  ...
  TreatmentTechniquesModule, TreatmentSessionLogModule, Sparkline, LiveSOAPPanel, ObservationModule, SOAPDocViewer };
