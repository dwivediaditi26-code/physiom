...
function ErgoModule({ data, set }) {
  ...
  const WorkstationTab = ()=>(
    <div>
      <SH id="ws_chair" label="Chair Ergonomics">
        <EF id="ergo_chair_height" label="Chair seat height" type="select" data={data} set={set} options={["Ideal - thighs parallel, feet flat","Too low - knees above hips","Too high - feet unsupported"]} note="Ideal: 90deg hip & knee, feet flat on floor or footrest"/>
        ...
        <EF id="ergo_armrest" label="Armrests" type="select" data={data} set={set} options={["Ideal - elbows 90deg, no shoulder elevation","Too high - shoulder shrug","Too low - lateral lean","Absent"]}/>
      ...
      <SH id="ws_input" label="Keyboard, Mouse & Input">
        <EF id="ergo_keyboard_pos" label="Keyboard position" type="select" data={data} set={set} options={["Ideal - elbows ~90deg, forearms neutral","Too high - shoulder elevation","Too far - trunk lean forward","Too close - restricted elbow angle"]}/>
        <FW id="ergo_keyboard_pos"/>
        <EF id="ergo_elbow_angle" label="Elbow angle at keyboard" type="select" data={data} set={set} options={["90-100deg (ideal)","<80deg (too acute)","110-120deg (moderate extension)","Full extension (>120deg)"]}/>
  ...
  const PostureTab = ()=>(
    <div>
      ...
      <SH id="ps_head" label="Head & Cervical">
        <EF id="ergo_head_angle" label="Forward head angle" type="range" min={0} max={40} step={5} unit="deg" data={data} set={set} note="0deg = ear over shoulder (ideal). >15deg = clinically significant. 30deg = ~18kg effective cervical load."/>
      ...
      <SH id="ps_shoulder" label="Shoulder & Upper Quarter">
        ...
        <EF id="ergo_shoulder_abd" label="Shoulder abduction angle at mouse" type="range" min={0} max={45} step={5} unit="deg" data={data} set={set} note="Ideal <15deg. >25deg = sustained rotator cuff load."/>
      </SH>
      <SH id="ps_lumbar" label="Lumbar & Pelvis">
        ...
        <EF id="ergo_hip_angle" label="Hip angle in seated position" type="select" data={data} set={set} options={["90-100deg (ideal)","<90deg - hip flexor shortened","110deg+ - posterior pelvic tilt risk","Asymmetric hip position"]}/>
      ...
      <SH id="ps_ul" label="Upper Limb & Wrist">
        <EF id="ergo_wrist_ext_angle" label="Wrist extension at rest" type="range" min={0} max={40} step={5} unit="deg" data={data} set={set} note="Ideal: 0-10deg. >15deg = carpal tunnel risk."/>
  ...
  const CORRECTIONS = {
    ...
    ergo_neck_rotation:   {priority:"High",  area:"Monitor",       action:"Centre primary monitor directly ahead. Adjust dual-monitor layout to within +/-35deg."},
    ergo_chair_height:    {priority:"High",  area:"Chair",         action:"Adjust chair: hips and knees 90-100deg, feet flat or footrest used."},
    ...
    ergo_keyboard_pos:    {priority:"High",  area:"Input",         action:"Position keyboard so elbows at 90deg and wrists neutral. Use keyboard tray if needed."},
    ...
    ergo_mouse_pos:       {priority:"High",  area:"Input",         action:"Move mouse immediately beside keyboard. Keep shoulder adducted <15deg during use."},
    ergo_elbow_angle:     {priority:"Medium", area:"Input",        action:"Adjust seating or keyboard height to achieve 90-100deg elbow flexion."},
    ...
    ergo_dual_monitor:    {priority:"Medium", area:"Monitor",      action:"Centre monitors equally OR set one primary directly ahead. Keep secondary within +/-35deg."},
  ...
  const PlanTab = ()=>{
    ...
    const movPx=[];
    if(risks.cervical==="High"||risks.cervical==="Moderate") movPx.push({label:"Cervical Mobility",freq:"Every 30 min",ex:["Chin tucks x10","Cervical rotation L+R x8","Cervical lateral flex x8","Upper trap stretch 30s each side"]});
    if(risks.lumbar==="High"||risks.lumbar==="Moderate")    movPx.push({label:"Lumbar Activation",freq:"Every 45 min",ex:["Seated pelvic clock x10","Hip flexor standing stretch 30s","Brief walk 2-3 min","Seated glute press x15"]});
    if(risks.ucs==="High"||risks.ucs==="Moderate")         movPx.push({label:"UCS Postural Reset",freq:"Every 20 min",ex:["Scapular retraction x10 (5s hold)","Thoracic extension over chair x5","Wall slide W-Y x10","DNF chin nod x10"]});
    if(risks.rsi==="High"||risks.rsi==="Moderate")         movPx.push({label:"Upper Limb Care",freq:"Every 60 min",ex:["Wrist flex/ext stretch 30s","Tendon glides x10","Forearm pronation/supination x15","Grip relaxation + intrinsic stretch"]});
    return (
      <div>
        {faults.length===0?(
    ...
    <div>
      ...
      {tab==="oga"&&<div>
        ...
        <SH id="oga_ant" label="Anterior View">
          ...
          {row("Foot progression angle","g_oga_foot_angle","select",["Neutral (5-10deg ER)","In-toeing","Out-toeing","Asymmetric"])}
        ...
        <SH id="oga_lat" label="Lateral View">
          ...
          {row("Hip ROM (sagittal)","g_oga_hip_rom","select",["Normal (40deg flex / 10deg ext)","Reduced flexion","Reduced extension","Both reduced"])}
          {row("Knee flexion pattern","g_oga_knee_flex","select",["Normal (0-60deg swing)","Stiff knee swing","Excess flexion","Hyperextension in stance"])}
      ...
      {tab==="timed"&&<div>
        {[
          ...
          {id:"g_5sts",  label:"5x Sit to Stand",     unit:"sec", normal:"<12s",               note:"From seated, stand fully 5 times without using arms if possible."},
      ...
      {tab==="muscles"&&<div>
        ...
        {row("ROM Restrictions","g_rom","textarea",null,"List joint: motion: degrees e.g. R hip ext 5deg (normal 10deg)")}
    ...
    <div>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ display:"flex", gap:6, marginBottom: showHistory ? 8 : 0 }}>
          ...
          {sessions.length > 0 && <button onClick={clearHistory} style={{ padding:"6px 10px", background:"transparent", border:"1px solid rgba(255,77,109,0.25)", borderRadius:8, color:"rgba(255,77,109,0.6)", fontSize:"0.62rem", cursor:"pointer" }}>x Clear</button>}
        </div>
...
function buildClinicalInterpretation(data) {
  ...
  romChecks.forEach(([key, label, norm]) => {
    ...
    if (!isNaN(v) && v > 0) {
      ...
      if (pct < 50) romSevere.push(`${label} ${v}deg/${norm}deg`);
      else if (pct < 80) romMild.push(`${label} ${v}deg/${norm}deg`);
    }
  });
...
function buildRealtimeSOAP(data, extraS="", extraO="", extraA="", extraP="") {
  ...
  // ── POSTURE / OBSERVATION ─────────────────────────────────────────────────
  {
    ...
    if (data.cvaAngle) sagLines.push(`CVA ${data.cvaAngle}deg`);
    if (data.sagThorKyph) sagLines.push(`Thoracic kyphosis ${data.sagThorKyph}deg`);
    if (data.sagLumLord) sagLines.push(`Lumbar lordosis ${data.sagLumLord}deg`);
  ...
  // ── ROM - correct field IDs from ROM_DATA ───────────────────────────────
  {
    ...
    if (slrL||slrR) romRows.push(`  SLR: L ${slrL||"-"} / R ${slrR||"-"} (norm >70deg)`);
  ...
  if (hepArr.length) {
    ...
    hepArr.forEach((ex,i) => P_parts.push(`  ${i+1}. ${ex.name} - ${ex.customSets||ex.sets}x${ex.customReps||ex.reps}, hold ${ex.customHold||ex.hold}s, ${ex.customFreq||ex.freq}${ex.notes?` (${ex.notes})`:""}`));
  }
...
const EXERCISE_DB = {
  cervical: {
    ...
    categories: {
      "Mobility & Flexibility": [
        ...
        { id:"cx_lat_flex",         name:"Lateral Neck Stretch",                    target:"Upper trapezius, SCM, scalenes, levator scapulae",  desc:"Tilt ear to shoulder. Opposite hand adds gentle overpressure. Hold 30s.",  sets:3, reps:1,  hold:30, freq:"3x/day",  phase:"Phase 1", evidence:"Moderate", cues:"Do not elevate the shoulder being stretched toward.",     progression:"Neural mobilisation add-on (depress contralateral shoulder)" },
      ...
      "Neural Mobilisation": [
        { id:"cx_neural_slider",    name:"Upper Limb Neural Slider (Median)",       target:"Median nerve - brachial plexus",                   desc:"Side-flex neck away + extend elbow + dorsiflex wrist simultaneously.",      sets:3, reps:10, hold:0,  freq:"2x/day",   phase:"Phase 2", evidence:"Moderate", cues:"Neck and hand move in OPPOSITE directions. No pain.",     progression:"Add shoulder abduction -> Tensioner technique" },
        { id:"cx_neural_ulnar",     name:"Upper Limb Neural Slider (Ulnar)",        target:"Ulnar nerve - C8/T1",                              desc:"Elbow flexion + wrist extension + neck lateral flex away. Slider.",         sets:3, reps:10, hold:0,  freq:"2x/day",   phase:"Phase 2", evidence:"Moderate", cues:"Smooth rhythmic movement. Stop if sharp or burning pain.", progression:"Add wrist deviation -> Tensioner" },
        { id:"cx_neural_radial",    name:"Upper Limb Neural Slider (Radial)",       target:"Radial nerve - C6/C7",                             desc:"Shoulder IR + elbow extension + wrist flex + neck lateral flex away.",      sets:3, reps:10, hold:0,  freq:"2x/day",   phase:"Phase 2", evidence:"Moderate", cues:"Rhythmic. Smooth and synchronised movements.",            progression:"Add shoulder depression -> Tensioner" },
      ],
    }
  },
  shoulder: {
    ...
    categories: {
      "Rotator Cuff": [
        { id:"sh_er_band",          name:"External Rotation with Band",             target:"Infraspinatus, teres minor",                       desc:"Elbow at side, 90deg flexion. Rotate outward against resistance.",            sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Elbow stays against body. Control the return.",           progression:"Increase resistance -> ER at 90deg abduction -> Prone ER" },
        { id:"sh_empty_can",        name:"Empty Can (Supraspinatus Isolation)",     target:"Supraspinatus",                                    desc:"Arm in scapular plane (30deg fwd), thumb down. Elevate to 90deg.",              sets:3, reps:12, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Stop if painful arc. Pain-free range initially.",         progression:"Full can (thumb up) -> Side-lying ER -> Overhead load" },
        { id:"sh_prone_er",         name:"Prone External Rotation 90/90",           target:"Infraspinatus, posterior cuff",                    desc:"Prone, arm 90deg abducted, elbow 90deg flexed. Rotate forearm upward.",         sets:3, reps:12, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Keep shoulder at 90deg. No trunk rotation.",                progression:"Add resistance -> Side-lying -> Standing cable" },
      ...
      "Scapular Stabilisation": [
        ...
        { id:"sh_sidelying_abd",    name:"Side-Lying Shoulder Abduction",           target:"Middle deltoid, supraspinatus - scapular plane",   desc:"Side-lying. Raise arm in scapular plane to 90deg. Control return.",           sets:3, reps:12, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Moderate", cues:"Thumb slightly up. Stay in pain-free arc.",               progression:"Add weight -> Standing lateral raise" },
      ...
      "Mobility": [
        { id:"sh_pendulum",         name:"Codman's Pendulum",                       target:"GH joint - passive decompression",                 desc:"Lean forward, arm hanging. Small circles using trunk momentum.",             sets:3, reps:20, hold:0,  freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Arm is PASSIVE. Let gravity do the work.",                progression:"Increase circle size -> Add 0.5-1kg" },
        { id:"sh_pec_stretch",      name:"Pectoralis Minor Stretch",                target:"Pectoralis minor, anterior capsule",                desc:"Doorway stretch. Arm at 90deg. Step through. Or supine on foam roller.",       sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Do not arch lower back. Core engaged.",                   progression:"Corner stretch -> Unilateral with scapular PT cue" },
        { id:"sh_pully",            name:"Shoulder Pulley AROM",                    target:"GH joint - ROM restoration",                       desc:"Overhead pulley. Use good arm to assist bad arm through range.",             sets:3, reps:15, hold:2,  freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Assisted - do not force range. Smooth movement.",         progression:"Reduce assistance -> AROM -> Add load" },
        { id:"sh_capsule_stretch",  name:"Inferior Capsule Stretch",                target:"Inferior GH capsule - frozen shoulder",            desc:"Supine. Hold arm at side, slightly abducted. Gentle traction downward.",     sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Gentle traction only. No pain reproduction.",             progression:"Increase hold -> Combine with ER stretches" },
      ],
    }
  },
  elbow: {
    ...
    categories: {
      ...
      "Medial Epicondylalgia": [
        ...
        { id:"el_forearm_stretch",  name:"Forearm Flexor Stretch",                  target:"Forearm flexors - medial epicondyle",               desc:"Elbow extended. Wrist extended. Gentle overpressure with other hand.",       sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Feel stretch in forearm - not elbow pain.",               progression:"Add forearm supination -> Neural component" },
      ],
      "Elbow Mobility": [
        { id:"el_pron_sup",         name:"Forearm Pronation-Supination",            target:"Pronator teres, supinator",                         desc:"Elbow at 90deg. Slowly pronate and supinate. Use dowel for feedback.",         sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Moderate", cues:"Keep elbow still. Move only at forearm.",                 progression:"Add resistance with weighted dowel" },
  ...
  wrist_hand: {
    ...
    categories: {
      "Wrist Rehabilitation": [
        ...
        { id:"wh_tendon_glide",     name:"Tendon Gliding Exercises",                target:"FDP, FDS, extensor tendons",                       desc:"Sequence: straight -> hook fist -> full fist -> flat fist -> tip-pinch. 5s each.", sets:3, reps:10, hold:5, freq:"3x/day", phase:"Phase 1", evidence:"Strong",    cues:"Move through each position fully. Slow and controlled.",  progression:"Add resistance -> Functional grip tasks" },
        { id:"wh_nerve_glide",      name:"Median Nerve Glide (CTS)",                target:"Median nerve - carpal tunnel",                     desc:"Wrist neutral. Extend fingers/wrist -> add neck lateral flex away. Slider.", sets:3, reps:10, hold:0,  freq:"2x/day",   phase:"Phase 2", evidence:"Moderate", cues:"No pain. Mild stretch only. Smooth movement.",            progression:"Tensioner technique if slider resolves" },
      ...
      "De Quervain's / Thumb": [
        ...
        { id:"wh_dq_stretch",       name:"Finkelstein Stretch (De Quervain's)",     target:"APL, EPB - De Quervain's syndrome",                desc:"Thumb in palm, fingers wrapped over. Ulnar deviate wrist gently. Hold.",    sets:3, reps:1,  hold:20, freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Gentle. Stop if sharp pain. Self-mobilisation only.",     progression:"Add gentle ulnar deviation in function" },
  ...
  lumbar: {
    ...
    categories: {
      "Core Stabilisation": [
        ...
        { id:"lb_dead_bug",         name:"Dead Bug",                                 target:"TA, anti-extension core",                          desc:"Supine, arms up, hips/knees 90deg. Lower opposite arm/leg toward floor.",     sets:3, reps:8,  hold:0,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Lower back must NOT lift. Exhale as you lower.",          progression:"Resistance band -> Both legs -> Add weight" },
  ...
  hip: {
    ...
    categories: {
      "Gluteal Strengthening": [
        { id:"hp_clam",             name:"Clamshells",                               target:"Gluteus medius, hip external rotators",            desc:"Side-lying, hips 45deg flexed. Rotate top knee up. Keep pelvis still.",        sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Do not rotate pelvis backward.",                          progression:"Resistance band -> Side-lying abduction -> Standing" },
        ...
        { id:"hp_standing_abd",     name:"Standing Hip Abduction",                   target:"Gluteus medius, minimus",                          desc:"Stand on one leg. Lift other leg out to side 30-40deg. Control return.",       sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Keep pelvis level. No Trendelenburg.",                    progression:"Add ankle weight -> Cable -> SL balance with abduction" },
      ...
      "Mobility & Flexibility": [
        { id:"hp_90_90",            name:"90/90 Hip Stretch",                        target:"Hip IR & ER - both hips",                          desc:"Seated on floor, both hips at 90deg. Sit tall. Hold 60s per side.",           sets:2, reps:1,  hold:60, freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Both sit bones on floor. Breathe.",                       progression:"Forward fold -> Lateral lean -> Dynamic transitions" },
        { id:"hp_pigeon",           name:"Pigeon Pose",                              target:"Piriformis, deep hip external rotators",           desc:"Front leg at 90deg. Back leg extended. Lower chest forward.",                  sets:3, reps:1,  hold:60, freq:"Daily",    phase:"Phase 1", evidence:"Moderate", cues:"Front foot flexed. Square hips.",                         progression:"Quad stretch add-on -> Supported -> Dynamic pigeon" },
        { id:"hp_thomas",           name:"Thomas Test Stretch",                      target:"Iliopsoas, rectus femoris",                        desc:"Supine at table edge. Hold one knee. Let other leg hang and feel stretch.", sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Lumbar flat. No external rotation of hanging leg.",       progression:"Add knee bend (RF) -> Standing lunge -> RNT lunge" },
  ...
  knee: {
    ...
    categories: {
      "Quadriceps": [
        { id:"kn_tqe",              name:"Terminal Knee Extension (TKE)",            target:"VMO - last 30deg extension",                         desc:"Band behind knee. Drive to full extension against resistance.",              sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Full extension - lock knee out. Keep foot on floor.",     progression:"Increase resistance -> Single-leg -> Mini-squat" },
        { id:"kn_vmo_squat",        name:"VMO Squat (Narrow Stance)",                target:"VMO - medial quadriceps",                          desc:"Narrow stance, toes slightly out. Squat to 60deg.",                            sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Track knee over 2nd toe. No knee cave.",                  progression:"Add weight -> Bulgarian split squat -> Single-leg" },
        ...
        { id:"kn_straight_leg",     name:"Straight Leg Raise",                       target:"Quadriceps (isometric), hip flexors",               desc:"Supine. Quad set first. Raise straight leg to 45deg. Hold 2s. Lower.",         sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Quad set BEFORE raising. Keep foot dorsiflexed.",         progression:"Add ankle weight -> Progress to TKE" },
        { id:"kn_leg_press",        name:"Leg Press (Short Arc)",                    target:"Quadriceps, glutes - controlled loading",          desc:"Leg press machine. Start from 90deg. Extend to full extension. Slow return.",  sets:3, reps:12, hold:0,  freq:"3x/week",  phase:"Phase 2", evidence:"Strong",    cues:"Push through whole foot. Control lowering 3-4s.",         progression:"Increase load -> Full range -> Single-leg" },
      ],
      "Hamstrings": [
        { id:"kn_nordic",           name:"Nordic Hamstring Curl",                    target:"Biceps femoris, semimembranosus - eccentric",      desc:"Kneel, feet anchored. Lower body slowly controlling with hamstrings.",       sets:3, reps:6,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strongest - 51% hamstring injury reduction", cues:"Lower as slowly as possible. Push up with hands.",        progression:"Increase reps -> Add resistance -> Glider curl" },
      ...
      "Patellar Tendinopathy": [
        { id:"kn_isometric_wall",   name:"Isometric Wall Sit",                       target:"Patellar tendon - isometric analgesic",            desc:"Back against wall. Squat at 60-90deg. Both legs.",                            sets:4, reps:1,  hold:45, freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"60-90deg knee bend. 70% effort. No pain >4/10.",            progression:"Single-leg -> Add weight -> Isotonic" },
        { id:"kn_slow_squat",       name:"Heavy Slow Resistance Squat",              target:"Patellar tendon - isotonic loading",               desc:"Slow tempo squat: 3s down, 3s up. Progressive load.",                        sets:4, reps:8,  hold:0,  freq:"3x/week",  phase:"Phase 2", evidence:"Strong - Kongsgaard protocol", cues:"3s down, 2s pause, 3s up. Progress load weekly.",         progression:"Increase load -> Single-leg -> Plyometric" },
        { id:"kn_decline_squat",    name:"Decline Board Squat",                      target:"Patellar tendon - high load eccentrics",           desc:"Stand on 25deg decline board. Single-leg squat. Slow eccentric.",              sets:3, reps:10, hold:0,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"Use hands for balance initially. Slow down.",             progression:"Increase decline -> Add load -> Hop landing" },
      ],
      "ACL Rehab": [
        { id:"kn_acl_phase1",       name:"Early ACL Quad Activation",                target:"Quadriceps - ACL graft protection",                desc:"Quad sets + SLR + TKE (0-90deg). Avoid open chain 0-45deg for 12 weeks.",       sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Avoid full extension open chain in early graft healing.", progression:"Progress range at 12 weeks -> Closed chain loading" },
        ...
        { id:"kn_drop_jump",        name:"Drop Jump Landing Training",               target:"Quadriceps, glutes - ACL prevention",              desc:"Step off box. Land softly with triple flexion. Hold 2s.",                    sets:3, reps:8,  hold:2,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Land soft - quiet feet. Hips back, knees tracking.",     progression:"Increase height -> Add lateral -> Sprint -> Cut" },
      ],
    }
  },
  ankle: {
    ...
    categories: {
      "Achilles Tendinopathy": [
        { id:"ank_ec_drop",         name:"Eccentric Heel Drop (Alfredson)",          target:"Gastrocnemius, soleus - eccentric",                desc:"Stand on step. Rise with both feet, lower with one. Full range.",            sets:3, reps:15, hold:0,  freq:"2x/day",   phase:"Phase 2", evidence:"Strongest - gold standard", cues:"Lower all the way. Use other foot to rise. Through PAIN.", progression:"Add backpack weight (10% BW) -> Increase weekly" },
        ...
        { id:"ank_heavy_slow_calf", name:"Heavy Slow Resistance Calf Raise",         target:"Achilles tendon - isotonic remodelling",           desc:"Seated (soleus) + standing (gastrocnemius). Slow 3:3 tempo with load.",      sets:4, reps:8,  hold:0,  freq:"3x/week",  phase:"Phase 2", evidence:"Strong",    cues:"3s up, 2s hold, 3s down. Progress load weekly.",          progression:"Increase load -> Single-leg -> Plyometric" },
      ],
      "Ankle Stability": [
        ...
        { id:"ank_lateral_hops",    name:"Lateral Hop Progression",                  target:"Peroneals, ankle stabilisers - reactive",          desc:"Hop laterally over line. Double-leg -> single-leg. Land softly.",             sets:3, reps:10, hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Quiet landing. Triple flexion. No valgus collapse.",     progression:"Increase distance -> Add forward component -> Speed" },
      ...
      "Plantar Fascia": [
        ...
        { id:"ank_pf_stretch",      name:"Plantar Fascia Stretch",                   target:"Plantar fascia, toe flexors",                      desc:"Cross foot over knee. Pull toes back. Hold.",                                sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Pull toes back - feel stretch in arch. Before first steps.", progression:"Wall toe stretch -> Towel stretch -> Calf combine" },
        { id:"ank_calf_stretch",    name:"Gastrocnemius Calf Stretch",               target:"Gastrocnemius, Achilles",                          desc:"Wall stretch. Rear leg straight. Heel on floor. Hold 30s.",                  sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Keep heel down. Knee straight.",                          progression:"Bent knee (soleus) -> Eccentric drops" },
  ...
  posture_correction: {
    ...
    categories: {
      "Upper Crossed Syndrome": [
        ...
        { id:"pc_pec_foam",         name:"Pec Stretch on Foam Roller",               target:"Pectoralis major & minor, anterior capsule",       desc:"Supine on foam roller. Arms out 90deg. Let gravity open chest.",               sets:1, reps:1,  hold:120,freq:"Daily",    phase:"Phase 1", evidence:"Moderate", cues:"Relax completely. Gravity does the work.",                progression:"Increase hold -> Add arm elevation -> IR/ER in position" },
        { id:"pc_levator_str",      name:"Levator Scapulae Stretch",                 target:"Levator scapulae - UCS component",                 desc:"Look down 45deg. Rotate head 45deg away. Side bend. Gentle overpressure.",       sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Moderate", cues:"Depression of opposite shoulder increases stretch.",      progression:"Add neural component -> Sustained hold -> Self-SNAG" },
      ...
      "Lower Crossed Syndrome": [
        ...
        { id:"pc_hip_flex_str",     name:"Kneeling Hip Flexor Stretch with PPT",     target:"Iliopsoas - LCS correction",                       desc:"Kneeling lunge. PPT first. Then lean forward into stretch.",                 sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Tilt pelvis first. Feel deep front of hip - not thigh.", progression:"Add thoracic rotation -> RNT lunge" },
      ...
      "Thoracic Mobility": [
        ...
        { id:"pc_book_open",        name:"Book Openings",                            target:"Thoracic rotators, pectorals",                     desc:"Side-lying, knees 90deg. Top arm opens chest to ceiling.",                    sets:3, reps:10, hold:3,  freq:"Daily",    phase:"Phase 1", evidence:"Strong",    cues:"Keep knees stacked. Let arm fall with gravity.",          progression:"Band resistance -> Quadruped rotation -> Standing" },
        { id:"pc_doorway_pec",      name:"Doorway Pec Minor Stretch",                target:"Pectoralis minor - forward shoulder correction",   desc:"Forearm against doorframe at 90deg. Step through. Lean into stretch.",         sets:3, reps:1,  hold:30, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Feel stretch in anterior chest. Don't arch lower back.",  progression:"Bilateral -> Change angle -> Dynamic add shoulder IR/ER" },
      ],
    }
  },
  respiratory: {
    ...
    categories: {
      ...
      "Respiratory Strengthening": [
        ...
        { id:"resp_acbt",           name:"Active Cycle of Breathing Technique",      target:"Secretion clearance - chest physiotherapy",        desc:"Breathing control -> 3-4 thoracic expansion exercises -> forced expirations.", sets:3, reps:1, hold:0,  freq:"2-3x/day", phase:"Phase 1", evidence:"Strong",    cues:"Relaxed breathing first. Sniff and huff - not cough.",   progression:"Add postural drainage -> Percussions -> Autogenic drainage" },
      ],
    }
  },
  neurological: {
    ...
    categories: {
      ...
      "Gait Training": [
        ...
        { id:"neuro_treadmill_bw",  name:"Body-Weight Supported Treadmill Training", target:"Lower limb motor - neurological gait",            desc:"Partial body-weight support. Repetitive gait practice.",                     sets:1, reps:1,  hold:0,  freq:"5x/week",  phase:"Phase 2", evidence:"Strongest - neuroplasticity and gait recovery", cues:"Step naturally. Reduce support progressively.",          progression:"Reduce support % -> Increase speed -> Overground" },
      ],
      "Motor Relearning": [
        { id:"neuro_task_practice", name:"Task-Specific Practice (Sit-to-Stand)",    target:"Motor learning - functional task",                 desc:"Repetitive sit-to-stand practice. Massed practice for neuroplasticity.",     sets:5, reps:10, hold:0,  freq:"3x/day",   phase:"Phase 2", evidence:"Strongest", cues:"Lean forward first. Equal weight. Stand tall.",           progression:"Vary chair height -> Add modifications -> Single-leg STS" },
        { id:"neuro_mirror",        name:"Mirror Therapy",                           target:"Motor cortex - phantom limb / CRPS",               desc:"Mirror box. Move intact limb. Brain sees reflection as affected limb.",      sets:3, reps:10, hold:0,  freq:"3x/day",   phase:"Phase 2", evidence:"Strong",    cues:"Focus on mirror - not on affected limb.",                progression:"Progress complexity -> Virtual reality" },
      ],
    }
  },
  pelvic_floor: {
    ...
    categories: {
      "Pelvic Floor Strengthening": [
        { id:"pf_kegel",            name:"Pelvic Floor Contraction (Kegel)",         target:"Levator ani, pubococcygeus - stress incontinence", desc:"Squeeze and lift pelvic floor. Hold 10s. Relax fully 10s. No breath holding.", sets:3, reps:10, hold:10, freq:"3x/day", phase:"Phase 1", evidence:"Strongest - gold standard", cues:"Squeeze UP and IN. Don't tighten buttocks or thighs.",   progression:"Increase hold -> Add quick flicks -> Functional positions" },
        { id:"pf_quick_flick",      name:"Quick Flick Contractions",                 target:"Type II pelvic floor fibres - urgency control",    desc:"Rapid squeeze and release. 1s on, 1s off.",                                  sets:3, reps:10, hold:0,  freq:"3x/day",   phase:"Phase 2", evidence:"Strong",    cues:"Fast on-off. No substitution patterns.",                  progression:"Increase reps -> Progress to functional activities" },
        ...
        { id:"pf_relaxation",       name:"Pelvic Floor Downtraining / Relaxation",   target:"Hypertonic pelvic floor - pain / vaginismus",     desc:"Diaphragmatic breath. On exhale consciously relax pelvic floor completely.", sets:3, reps:10, hold:10, freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Let go completely. Soft belly. Soft pelvic floor.",       progression:"Add visualisation -> Supine -> Seated -> Standing" },
      ],
      "Pelvic Girdle / SIJ": [
  ...
  older_adult: {
    ...
    categories: {
      ...
      "Sarcopenia Prevention": [
        { id:"oa_resistance",       name:"Progressive Resistance Training (Older Adult)", target:"Muscle mass, bone density - sarcopenia",     desc:"Major muscle groups. 60-70% 1RM. 2-3 sets. 8-12 reps. 2-3x/week.",        sets:3, reps:10, hold:0,  freq:"3x/week",  phase:"Phase 2", evidence:"Strongest", cues:"Safe technique. Slow controlled. No breath-holding.",     progression:"Increase load 5% when achieving 15 reps x 2 sets" },
        { id:"oa_power_training",   name:"Power Training (Older Adult)",             target:"Fast-twitch fibres - fall recovery speed",         desc:"Squat with faster concentric phase. Same controlled lowering.",              sets:3, reps:8,  hold:0,  freq:"2x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Fast up - slow down. Control at all times.",              progression:"Sit-to-stand fast -> Power squat -> Step-up fast" },
      ],
    }
  },
  sports: {
    ...
    categories: {
      "Return to Running": [
        ...
        { id:"sp_plyometric",       name:"Plyometric Progression",                   target:"Lower limb - reactive strength",                   desc:"Double-leg -> single-leg -> lateral -> rotational hops.",                      sets:3, reps:10, hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Land softly - triple flexion. Quiet feet.",               progression:"Double -> Single -> Lateral -> Rotational -> Sport-specific" },
        { id:"sp_agility",          name:"Agility & Change of Direction Drills",     target:"Lower limb - neuromuscular control, RTS",          desc:"T-test, 5-10-5, figure-8. Controlled then max speed.",                       sets:3, reps:5,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Controlled first. Quality > speed initially.",            progression:"Increase speed -> Add sport ball -> Reactive" },
        { id:"sp_sprinting",        name:"Sprinting Progression",                    target:"Hamstrings, glutes - RTS max velocity",            desc:"60% -> 70% -> 80% -> 90% -> 100% max velocity.",                               sets:4, reps:5,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"No pain. Full hip extension. Maintain form.",             progression:"Linear -> Curved -> Reactive -> Sport-specific" },
      ],
      "Throwing / Upper Limb Sports": [
        { id:"sp_ir_er_ratio",      name:"IR/ER Ratio Strength Training",            target:"Rotator cuff balance - thrower's shoulder",        desc:"Maintain ER:IR strength ratio >66%. Prioritise ER.",                         sets:3, reps:15, hold:2,  freq:"Daily",    phase:"Phase 2", evidence:"Strong",    cues:"External rotation is the priority for overhead athletes.", progression:"Sidelying -> 90deg abduction -> Overhead" },
      ...
      "Strength & Conditioning": [
        { id:"sp_split_squat",      name:"Bulgarian Split Squat",                    target:"Quadriceps, glutes - unilateral strength",         desc:"Rear foot elevated. Front foot forward. Drop back knee toward floor.",       sets:4, reps:8,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Front shin vertical. Drop straight down. No knee cave.", progression:"Add weight -> Add deficit -> Jump split squat" },
        { id:"sp_bench_press",      name:"Bench Press Progression",                  target:"Pectorals, anterior deltoid, triceps",             desc:"Barbell bench. Scapulae retracted. Controlled lowering. Drive up.",          sets:4, reps:8,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Elbows 45-75deg from torso. Control descent.",             progression:"Add load -> Dumbbell -> Incline -> Decline" },
        { id:"sp_chin_up",          name:"Chin-Up / Pull-Up Progression",            target:"Latissimus dorsi, biceps, mid trap",               desc:"Full hang to chin over bar. Control lowering. Various grips.",               sets:4, reps:6,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Full range. Dead hang to start. Depress scapulae first.", progression:"Band assisted -> Bodyweight -> Weighted -> L-sit" },
        { id:"sp_trap_bar",         name:"Trap Bar Deadlift",                        target:"Total lower chain - safe primary mover pattern",   desc:"Trap bar. Neutral spine. Full hip extension. Progressively loaded.",         sets:4, reps:6,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strong",    cues:"Push floor away. Drive hips forward. Stand tall.",        progression:"Increase load -> Single-leg RDL -> Sumo deadlift" },
        ...
        { id:"sp_nordics",          name:"Nordic Hamstring Curl",                    target:"Biceps femoris - eccentric injury prevention",     desc:"Kneel, feet anchored. Lower body slowly with hamstrings.",                   sets:3, reps:6,  hold:0,  freq:"3x/week",  phase:"Phase 3", evidence:"Strongest - 51% hamstring injury reduction", cues:"Lower as slowly as possible. Push up with hands.",        progression:"Increase reps -> Add resistance -> Glider curl" },
      ],
    }
  },
  pilates_yoga: {
  ...
  hydrotherapy: {
    ...
    categories: {
      "Aquatic Rehabilitation": [
        { id:"hydro_walk",          name:"Aquatic Walking",                          target:"Lower limb - unloaded gait retraining",            desc:"Chest-deep water walking. Forward, backward, sideways. Normal gait.",        sets:3, reps:1,  hold:0,  freq:"3x/week",  phase:"Phase 1", evidence:"Strong",    cues:"Normal heel-toe gait. Arms swing naturally.",             progression:"Increase speed -> Deep water -> Add buoyancy resistance" },
        { id:"hydro_squat",         name:"Aquatic Squat",                            target:"Quadriceps, glutes - reduced load",                desc:"Waist-deep water. Squat as normal - water reduces load ~50%.",               sets:3, reps:15, hold:2,  freq:"3x/week",  phase:"Phase 1", evidence:"Strong",    cues:"Same form as land squat. Use buoyancy - don't fall.",     progression:"Deeper water -> Add noodle resistance -> Plyometric" },
        { id:"hydro_run",           name:"Deep Water Running",                       target:"Cardiovascular - offloading for injury",           desc:"Deep water with floatation belt. Running motion - no floor contact.",         sets:3, reps:1,  hold:300,freq:"3-5x/week", phase:"Phase 2", evidence:"Strong",   cues:"Upright posture. Full running pattern.",                  progression:"Increase duration -> Increase intensity -> Resistance bands" },
        { id:"hydro_balance",       name:"Single-Leg Balance in Water",              target:"Proprioception - aquatic reduced-load",            desc:"Single-leg stance in shallow water. Gentle wave perturbation.",              sets:3, reps:1,  hold:30, freq:"3x/week",  phase:"Phase 1", evidence:"Moderate", cues:"Near pool edge. Focus on fixed point.",                   progression:"Add reach tasks -> Close eyes -> Deeper water" },
        { id:"hydro_kick",          name:"Aquatic Leg Kicks / Flutter Board",        target:"Hip flexors, quadriceps, hip extensors - low load", desc:"Hold flutter board. Kick front crawl motion. Supported on water.",          sets:3, reps:1,  hold:120,freq:"3x/week",  phase:"Phase 1", evidence:"Moderate", cues:"Full hip extension on kick. Flutter - not slap.",         progression:"Increase duration -> Add ankle weights -> Depth change" },
      ],
    }
  },
  cardiac: {
    ...
    categories: {
      "Phase 2-3 Cardiac Rehab": [
        ...
        { id:"card_resistance",     name:"Cardiac Resistance Training",              target:"Peripheral muscle - cardiac load reduction",       desc:"Circuit training at 40-60% 1RM. 8-10 exercises. No Valsalva.",              sets:2, reps:12, hold:0,  freq:"2-3x/week", phase:"Phase 2", evidence:"Strong",   cues:"No breath-holding. Exhale on exertion. Monitor HR/BP.",  progression:"Increase reps -> Increase load -> Reduce rest time" },
        { id:"card_interval",       name:"High Intensity Interval Training (HIIT)", target:"VO2 max - Phase 3 cardiac (supervised only)",      desc:"4x4 min at 85-95% HRmax with 3 min active recovery. Supervised.",           sets:4, reps:1,  hold:240,freq:"3x/week",  phase:"Phase 3", evidence:"Strongest - superior VO2 gains vs MICT", cues:"Supervised only. ECG monitored. Stop if symptoms.",       progression:"Increase sessions -> Unsupervised home program" },
        { id:"card_stretching",     name:"Cool-Down Stretching",                    target:"Flexibility, autonomic recovery - post-cardiac exercise", desc:"Major muscle groups. Gentle static stretching post exercise.",       sets:1, reps:1,  hold:30, freq:"5x/week",  phase:"Phase 1", evidence:"Moderate", cues:"Never skip cool-down. Gentle only. Breathe.",             progression:"Increase hold -> Add mindfulness breathing" },
      ],
    }
  },
  oncology: {
    ...
    categories: {
      "Cancer Fatigue Management": [
        ...
        { id:"onco_aerobic",        name:"Supervised Aerobic Training",              target:"Cardiorespiratory fitness - cancer rehab",          desc:"Walking or cycling at moderate intensity. 150 min/week.",                    sets:1, reps:1,  hold:0,  freq:"5x/week",  phase:"Phase 2", evidence:"Strongest - reduces fatigue, improves survival", cues:"Rate exertion 5-6/10. Stop if dizzy or chest pain.",     progression:"Increase duration -> Resistance training add-on" },
        { id:"onco_resistance",     name:"Progressive Resistance Training - Oncology", target:"Muscle strength - cancer cachexia prevention",   desc:"2 sets major muscle groups. 60-70% 1RM. Supervised initially.",              sets:2, reps:12, hold:0,  freq:"2-3x/week", phase:"Phase 2", evidence:"Strong",   cues:"Monitor blood counts. Avoid when neutropenic.",          progression:"Progress load 5% weekly -> Functional integration" },
        { id:"onco_lymph_pump",     name:"Lymphatic Pump Exercises",                 target:"Lymphatic return - post-mastectomy / lymphoedema", desc:"Shoulder pumping. Elevation. Elbow flexion/extension. Deep breathing.",      sets:3, reps:20, hold:0,  freq:"3x/day",   phase:"Phase 1", evidence:"Strong",    cues:"Wear compression garment. Elevate limb.",                 progression:"Add progressive resistance -> Decongestive therapy" },
      ],
    }
  },
  paediatric: {
...
const KNEE_PROTOCOLS = [
    ...
    phases:[
      {
        ...
        color:"#00c97a",
        exercises:[
          { name:"Quadriceps Setting (Quad Sets)", sets:3, reps:15, hold:5, freq:"3x/day", desc:"Lie flat. Tighten quad by pushing knee into bed. Hold 5s. VMO activation without joint load. Essential first step post-flare.", cues:"Feel the quad tighten above the kneecap. No movement needed.", evidence:"Strong - initiates VMO in swollen/inhibited knee" },
          { name:"Straight Leg Raise (SLR)", sets:3, reps:15, hold:2, freq:"Daily", desc:"Lie flat. Tighten quad first, then raise leg to 45deg. Hold 2s. Lower slowly. Strengthens quad without knee compression.", cues:"Lock the knee fully before lifting. Slow return.", evidence:"Strong - safe VMO loading in early OA" },
          { name:"Inner Range Quads (IRQ)", sets:3, reps:15, hold:5, freq:"Daily", desc:"Seated, place rolled towel under knee. Extend leg from 30deg to full extension. Targets VMO specifically in the last 30deg of extension.", cues:"Squeeze the quad hard at the top. Hold 5 seconds.", evidence:"Strong - VMO isolation in terminal extension" },
          { name:"Ankle Pumps + Calf Raises (seated)", sets:3, reps:20, hold:0, freq:"Hourly", desc:"Seated. Pump ankles x 20, then rise onto toes x 20. Circulation and calf pump for swelling management.", cues:"Keep heels down for pumps. Rise tall for calf raises.", evidence:"Strong - oedema management in acute OA flare" },
        ]
      },
      {
        ...
        color:"#ffb300",
        exercises:[
          { name:"Terminal Knee Extension (TKE) - Band", sets:3, reps:20, hold:2, freq:"Daily", desc:"Band behind knee, loop fixed. Stand in slight knee bend. Extend knee against band resistance. Last 30deg of extension. Best VMO exercise in weight-bearing.", cues:"Push knee straight slowly. Hold 2s. Control return. Don't hyperextend.", evidence:"Strong - VMO in functional WB position" },
          { name:"Wall Slide / Mini Squat (0-45deg)", sets:3, reps:15, hold:2, freq:"Daily", desc:"Back against wall. Slide down to 45deg ONLY (avoid deep knee bend in OA). Weight even. Functional quad load within pain-free range.", cues:"Knees track over 2nd toe. Equal weight both feet. Stop at 45deg.", evidence:"Strong - OARSI recommended OA exercise" },
      ...
      },
      {
        ...
        color:"#ff4d6d",
        exercises:[
          { name:"Single-Leg Press (45deg - partial range)", sets:3, reps:12, hold:1, freq:"3x/wk", desc:"Leg press machine. Single leg. 0-60deg range. Progressive load. Best gym exercise for OA - controls range, loads quad and glute.", cues:"Don't lock the knee at full extension. Push through heel.", evidence:"Strong - progressive overload for OA" },
          { name:"Lateral Step-Down (eccentric control)", sets:3, reps:10, hold:0, freq:"3x/wk", desc:"Stand on step. Lower non-affected foot slowly toward floor (3-5s). Eccentric quad + hip control. Hardest functional exercise for knee.", cues:"Knee tracks over 2nd toe. Pelvis level. Count 4 seconds down.", evidence:"Strong - functional eccentric control" },
          ...
          { name:"Aquatic Exercise (hydrotherapy)", sets:3, reps:15, hold:0, freq:"2x/wk", desc:"Pool exercises - walking in water, leg raises, mini squats in water. Offloads 50% body weight. Ideal for BMI>30 or severe OA.", cues:"Warm water preferred. Buoyancy reduces load. Can do deeper squats safely.", evidence:"Strong - Cochrane review: significant pain reduction" },
        ]
      },
    ],
    treatment:[
  ...
  },
  {
    ...
    phases:[
      {
        ...
        color:"#00c97a",
        exercises:[
          { name:"VMO Isolation - Inner Range Quads", sets:3, reps:20, hold:5, freq:"Daily", desc:"Towel under knee at 30deg. Contract VMO (feel above inner kneecap). Extend to full. VMO fires last 30deg - most important phase for PF tracking.", cues:"Place fingers on VMO (inner quad, above knee). Feel it fire. Hold 5s.", evidence:"Strong - VMO retraining primary for PFPS" },
          { name:"Straight Leg Raise (SLR)", sets:3, reps:20, hold:2, freq:"Daily", desc:"Quad set first, then raise to 45deg. Safe loading without PF compression. Foundation exercise.", cues:"Tighten quad before lifting. No knee flexion during movement.", evidence:"Strong - PFPS phase 1 standard" },
      ...
      },
      {
        ...
        color:"#ffb300",
        exercises:[
          { name:"TKE with Band (VMO in WB)", sets:3, reps:20, hold:2, freq:"Daily", desc:"Band behind knee. Stand. Extend knee from 30deg against band. Most functional VMO exercise - weight bearing position.", cues:"Push knee back slowly. Squeeze VMO at end. Don't hyperextend.", evidence:"Strong - VMO in functional position" },
          ...
          { name:"Wall Squat with Ball (knees out)", sets:3, reps:15, hold:3, freq:"Daily", desc:"Wall slide with pilates ball between knees. Squeeze ball to activate adductors/VMO. Limits knee valgus. 0-60deg only for PFPS.", cues:"Squeeze ball, press knees outward, back flat on wall. 60deg max.", evidence:"Strong - VMO + medial loading for PF tracking" },
    ...
    treatment:[
      ...
      { name:"Patellar Mobilisation (medial glide)", desc:"Grade III medial patellar glide. 3 x 30 sec. Stretches tight lateral retinaculum. Reduces lateral PF tracking.", evidence:"Moderate - combined with exercise: strong evidence" },
      ...
      { name:"Soft Tissue - IT Band / TFL", desc:"Foam roll TFL x 90 sec. Cross-fibre massage lateral retinaculum. Reduces lateral pull on patella.", evidence:"Moderate - adjunct to exercise" },
    ]
  },
  {
    id:"knee_hamstring",
    ...
    phases:[
      {
        ...
        color:"#00c97a",
        exercises:[
          { name:"Isometric Hamstring Hold", sets:5, reps:1, hold:45, freq:"Daily", desc:"Prone, knee 30deg. Push foot into therapist hand (or wall). Isometric hold 45s x 5 reps. Best pain relief in acute tendinopathy.", cues:"No movement. Just push and hold. Pain 0-3/10 acceptable.", evidence:"Strong - Rio et al 2015: isometrics reduce tendon pain immediately" },
          { name:"Prone Hip Extension (glute dominant)", sets:3, reps:15, hold:2, freq:"Daily", desc:"Prone. Extend hip with knee bent 90deg. Activates glute max > hamstring. Offloads tendon while maintaining neural drive.", cues:"Squeeze glute. Feel the glute, not the hamstring. Knee stays at 90deg.", evidence:"Strong - glute loading offloads proximal hamstring" },
        ]
      },
      {
        ...
        color:"#ffb300",
        exercises:[
          { name:"Deadlift (Romanian - hip hinge)", sets:3, reps:10, hold:0, freq:"3x/wk", desc:"Hip hinge pattern. Load through hip, not lumbar spine. Progresses from bodyweight to dumbbell. Primary hamstring tendon loading exercise.", cues:"Hinge at hip. Feel tension in hamstrings before lowering. Flat back.", evidence:"Strong - Goom et al 2016: progressive loading for proximal hamstring tendinopathy" },
          { name:"Bridge - Single Leg (slow eccentric)", sets:3, reps:12, hold:0, freq:"3x/wk", desc:"Single leg bridge with 3s eccentric lower. Hip at full extension loads hamstring-glute junction. Progress: straight leg (hamstring dominant).", cues:"Push through heel. Squeeze glute at top. Lower for 3 seconds.", evidence:"Strong - hamstring-glute loading in functional position" },
          { name:"Leg Curl - Prone (isotonic)", sets:3, reps:15, hold:1, freq:"3x/wk", desc:"Prone leg curl machine. Full range, slow eccentric (3s). Isolated hamstring load. Important for distal tendon.", cues:"Control the return - 3 seconds lower. Don't let the weight drop.", evidence:"Strong - eccentric bias for tendinopathy" },
        ]
      },
      {
        ...
        color:"#ff4d6d",
        exercises:[
          { name:"Nordic Hamstring Curl", sets:3, reps:6, hold:0, freq:"2x/wk", desc:"Kneel, feet anchored. Lower body as slow as possible (eccentric). Most powerful hamstring strengthening exercise. Reduces hamstring injury 51%.", cues:"Go as slow as possible. Use hands to catch at bottom. Pull yourself back up.", evidence:"Strong - Petersen et al 2011: 51% injury reduction" },
          { name:"Hip Thrust with Bar (heavy)", sets:4, reps:8, hold:1, freq:"3x/wk", desc:"Barbell hip thrust. Full hip extension load. Targets glute max + hamstring junction. Progress to >80% body weight.", cues:"Full hip extension at top. Squeeze glute. Chin tucked. Drive through heels.", evidence:"Strong - high glute-ham loading for return to sprint" },
          { name:"Sprinting - Progressive", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Start 50% speed x 6 reps x 30m. Progress 10%/week. Pain <2/10. Do NOT sprint through pain.", cues:"Warm up well. First sprint always easy. Stop if any twinge at proximal hamstring.", evidence:"Strong - sport-specific loading final stage" },
        ]
      },
    ],
    treatment:[
      ...
      { name:"Shockwave Therapy (ESWT)", desc:"Radial ESWT x 2000 pulses x 3 sessions, 1 week apart. Applied directly to ischial tuberosity. Best for chronic (>3 months) proximal hamstring tendinopathy.", evidence:"Strong - Furia 2009, multiple RCTs" },
    ...
    phases:[
      {
        ...
        exercises:[
          ...
          { name:"Heel Slides (ROM recovery)", sets:3, reps:20, hold:1, freq:"3x/day", desc:"Supine. Slide heel toward buttock. Regain flexion ROM gently. Target 90deg by week 2. Avoid forced flexion.", cues:"Slide slowly. Stop at resistance or pain >3/10. Ice after.", evidence:"Strong - ROM recovery pacing post ACL-R" },
          ...
          { name:"Ankle Pumps + Calf Raises", sets:0, reps:20, hold:0, freq:"Hourly", desc:"DVT prevention. Hourly in first 2 weeks. Foot/ankle pump circulation.", cues:"Pump ankles x 20 then point and hold. Do every hour when awake.", evidence:"Strong - DVT prophylaxis post surgery" },
        ]
      },
      {
        ...
        color:"#ffb300",
        exercises:[
          { name:"Leg Press (double to single leg)", sets:3, reps:15, hold:1, freq:"3x/wk", desc:"Start double leg, progress to single. 0-70deg only initially (avoid 0deg full ext if graft tension issue). Progress load weekly.", cues:"Push through heel. Full control both ways. Progress range as tolerated.", evidence:"Strong - primary quad + glute loading post ACL-R" },
      ...
      },
      {
        ...
        color:"#ff4d6d",
        exercises:[
          { name:"Nordic Hamstring Curl", sets:3, reps:6, hold:0, freq:"2x/wk", desc:"Hamstring strength >= 80% of quad (H:Q ratio). Nordic curl builds hamstring as secondary ACL restraint. Cannot return to sport without adequate H:Q ratio.", cues:"Slow as possible. Build tolerance. Full programme takes 6 weeks.", evidence:"Strong - H:Q ratio normalisation for ACL return to sport" },
          { name:"Plyometric Progression (hop tests)", sets:3, reps:8, hold:0, freq:"3x/wk", desc:"Double leg to single leg to tuck jumps to lateral hops. LSI (limb symmetry index) must be >90% before RTS. Hop test battery: single hop, triple hop, crossover hop.", cues:"Soft landing. Knee over toe. Quiet landing = good absorption. Loud = poor control.", evidence:"Strong - Ardern et al: LSI >90% required for safe RTS" },
          { name:"Running Programme - Return to Run", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Criteria for running: full ROM, no effusion, quad strength >70% contralateral. Start: walk-jog intervals x 20 min. Progress weekly. No cutting/pivoting until month 6.", cues:"Pain <2/10. No swelling after. Increase volume before intensity.", evidence:"Strong - progressive return to run protocol" },
          { name:"Agility + Change of Direction", sets:3, reps:6, hold:0, freq:"3x/wk", desc:"T-drill, lateral shuffle, figure-8. Introduce at month 6+ only. Neuromuscular control under speed.", cues:"Start slow. Accelerate when technique is clean. Video from front for valgus check.", evidence:"Strong - sport-specific neuromuscular training pre-RTS" },
        ]
      },
    ],
    treatment:[
      { name:"Cryotherapy (Ice) - Phase 1", desc:"Ice x 15-20 min every 2 hours for first 2 weeks. Reduces post-op effusion which drives VMO inhibition (AMI).", evidence:"Strong - effusion management = VMO inhibition reversal" },
    ...
    phases:[
      ...
      },
      {
        ...
        exercises:[
          ...
          { name:"Single-Leg Squat (5cm step)", sets:3, reps:12, hold:2, freq:"Daily", desc:"Stand on 5cm step. Single leg squat to 30deg. Control knee valgus. This is the key functional test AND exercise for ITBS.", cues:"Knee straight over 2nd toe. Pelvis level. Stop if valgus occurs.", evidence:"Strong - functional hip + quad loading for ITBS" },
          { name:"Running Cadence Increase (10%)", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Increase running cadence 10% using metronome (app). Reduces hip adduction and stride length - directly reduces IT band tension.", cues:"Count steps. Use metronome app. Smaller, quicker steps. Same pace.", evidence:"Strong - Noehren 2011: cadence increase reduces IT band load 19%" },
          { name:"Downhill Avoidance + Gradual Return", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"ITBS is worse downhill (IT band compresses at 30deg flexion). Avoid hills for 6 weeks. Return flat first, gentle hills last.", cues:"Start flat. Treadmill incline 0%. Add hills at week 8 only.", evidence:"Strong - load management for IT band impingement" },
        ]
      },
    ],
    treatment:[
      { name:"TFL / IT Band Foam Roll", desc:"Foam roll lateral thigh 90 sec. NOT directly on IT band (too painful). Roll TFL at hip more than band itself. 2x daily.", evidence:"Moderate - reduces TFL tone, indirect IT band tension relief" },
      { name:"Hip Flexor Stretch (couch stretch)", desc:"Couch stretch x 3 x 60 sec. Tight hip flexors increase anterior pelvic tilt = increased IT band tension.", evidence:"Moderate - hip flexor length restoration" },
    ...
    phases:[
      {
        ...
        color:"#00c97a",
        exercises:[
          { name:"Isometric Leg Extension (70deg knee bend)", sets:5, reps:1, hold:45, freq:"Daily (pre-sport)", desc:"Leg extension machine. Knee at 70deg. Push maximally without moving. 45 second hold. Immediate analgesia effect (10 min). Do before training.", cues:"Maximum effort push. No movement. 45 seconds. Pain 0-4/10 acceptable.", evidence:"Strong - Rio et al 2015: isometrics = pain relief + cortical inhibition" },
          { name:"Decline Board Squat - Isometric Hold", sets:5, reps:1, hold:45, freq:"Daily", desc:"Stand on 25-35deg decline board. Single leg squat hold at 60deg. Isometric. More functional than leg extension for athletes.", cues:"Lean forward (decline helps). Hold the position. No bouncing. Feel the tendon load.", evidence:"Strong - most effective isometric loading for patellar tendinopathy" },
        ]
      },
      {
        ...
        color:"#ffb300",
        exercises:[
          { name:"Decline Single-Leg Squat - HSR Protocol", sets:4, reps:8, hold:0, freq:"3x/wk (NOT daily)", desc:"Heavy Slow Resistance (HSR) protocol. Decline board 25deg. Single leg. 3s down, 3s up. Load with vest/barbell. Kongsgaard 2009: HSR = best long-term tendinopathy outcome.", cues:"3 seconds down, 3 seconds up. SLOW is the key. Load heavily enough to limit to 8 reps.", evidence:"Strong - Kongsgaard 2009: HSR > corticosteroid at 6 months" },
          { name:"Leg Press (single leg - slow)", sets:4, reps:8, hold:0, freq:"3x/wk", desc:"Slow leg press. 3s eccentric, 3s concentric. Heavier than traditional. Progresses to sport-specific strength.", cues:"Count 3 seconds each way. Heavy enough that last 2 reps are hard.", evidence:"Strong - HSR protocol for tendinopathy" },
          { name:"Spanish Squat (wall squat - isometric)", sets:4, reps:1, hold:45, freq:"Daily between HSR days", desc:"Band around fixed post + behind back. Squat to 60deg. Push knees out against band. Long-duration isometric for pain days / maintenance.", cues:"Knees out against band. Back upright. Feel quad. Hold 45 seconds.", evidence:"Strong - adjunct to HSR on rest days" },
        ]
      },
      {
        ...
        color:"#ff4d6d",
        exercises:[
          { name:"Box Jump (double to single leg)", sets:4, reps:6, hold:0, freq:"2x/wk", desc:"Load the tendon with energy storage (plyometric). Start double leg. Progress to single leg box jump. Only introduce after HSR for 8 weeks, pain <2/10.", cues:"Land soft. Absorb. Pause. Don't rush. Pain <2/10 only.", evidence:"Strong - energy storage loading for patellar tendon RTS" },
          { name:"Depth Jump + Squat Jump", sets:3, reps:6, hold:0, freq:"2x/wk", desc:"Drop off box then immediate explosive jump. Maximum reactive strength for tendon. Sport-specific for jumpers/runners.", cues:"Ground contact <0.2 sec. Think: hot coals. Explosive. Only if pain-free.", evidence:"Strong - reactive tendon loading for return to jumping sports" },
        ]
      },
    ],
    treatment:[
      { name:"ESWT - Shockwave", desc:"Radial or focused ESWT x 2000 pulses per session x 3 sessions. Apply to patellar tendon. Pain 5-7/10 during acceptable. Best for chronic tendinopathy >3 months.", evidence:"Strong - Zwerver 2011 RCT, multiple systematic reviews" },
...
const SHOULDER_PROTOCOLS = [
  {
    ...
    phases:[
      { phase:"Phase 1 - Pain Control & Motor Control (Weeks 1-4)", color:"#00c97a", exercises:[
        ...
        { name:"Isometric External Rotation (neutral)", sets:5, reps:1, hold:45, freq:"Daily", desc:"Stand side-on to wall. Elbow at 90deg, pressed against side. Push hand gently outward into wall. Isometric - no movement. Immediate analgesic effect.", cues:"Maximum effort push - no movement. 45 seconds. Pain 0-4/10 acceptable.", evidence:"Strong - Rio 2015 isometric protocol adapted for shoulder RCT" },
      ...
      { phase:"Phase 2 - Strength & Load Tolerance (Weeks 4-10)", color:"#ffb300", exercises:[
        { name:"Side-Lying External Rotation", sets:3, reps:15, hold:2, freq:"Daily", desc:"Lie on non-affected side. Elbow at 90deg, forearm resting on abdomen. Rotate forearm up toward ceiling. Best isolation exercise for infraspinatus + teres minor.", cues:"Elbow stays glued to side. Rotate slowly. No shrug. 2s hold at top.", evidence:"Strong - Reinold 2004: highest infraspinatus EMG activation" },
        { name:"Prone Y - Lower Trapezius", sets:3, reps:15, hold:3, freq:"Daily", desc:"Lie face down, arm at 135deg (Y position). Raise arm toward ceiling. Thumb up. Lower trapezius activation essential for scapular upward rotation.", cues:"Thumb pointing up. Squeeze scapula down before lifting. Don't shrug.", evidence:"Strong - lower trap isolation for scapular control" },
        { name:"Prone T - Middle Trapezius", sets:3, reps:15, hold:3, freq:"Daily", desc:"Lie face down, arms at 90deg (T position). Raise both arms. Middle trap + rhomboid activation. Corrects scapular protraction pattern.", cues:"Arms at 90deg exactly. Lift only as high as pain-free. Hold 3s.", evidence:"Strong - Ekstrom 2003: optimal mid-trap activation" },
        { name:"Band External Rotation (0deg abduction)", sets:3, reps:20, hold:2, freq:"Daily", desc:"Band anchored at elbow height. Elbow at side, 90deg flexion. Rotate outward against band. Concentric + eccentric control.", cues:"Elbow stays at side. Slow return (3 seconds in). Keep shoulder blade down.", evidence:"Strong - infraspinatus + teres minor progressive loading" },
        { name:"Full Can (Scaption) - Band/Weight", sets:3, reps:15, hold:1, freq:"Daily", desc:"Arm raised in plane of scapula (30deg forward of coronal plane), thumb up. Raise to shoulder height only. Supraspinatus loading in safest arc.", cues:"Thumb up. 30deg forward of side. Raise to 90deg only. No shrug.", evidence:"Strong - Kelly 1996: full can = optimal supraspinatus with least impingement" },
      ...
      { phase:"Phase 3 - Function & Return to Activity (Weeks 10+)", color:"#ff4d6d", exercises:[
        ...
        { name:"Diagonal PNF Pattern (D2 Flexion)", sets:3, reps:12, hold:0, freq:"3x/wk", desc:"With band/cable. Start: arm across body low. Move diagonally up and out to end range. Functional pattern used in throwing, reaching overhead.", cues:"Full diagonal. Thumb leads upward. Smooth arc. No pain >3/10.", evidence:"Strong - PNF D2 pattern: functional overhead rehabilitation" },
        { name:"Wall Slide with Upward Rotation", sets:3, reps:15, hold:2, freq:"Daily", desc:"Forearms on wall, elbows at 90deg. Slide arms up while maintaining scapular upward rotation. Challenges serratus + lower trap at end-range overhead.", cues:"Maintain forearm contact. Feel shoulder blade rotate - not just shrug.", evidence:"Strong - overhead scapular control for return to sport/work" },
      ]},
    ],
    treatment:[
      { name:"Posterior Capsule Stretch (Sleeper Stretch)", desc:"Sidelying on affected side. Bring forearm down toward floor (internal rotation). 3 x 30 sec. Tight posterior capsule causes anterosuperior humeral head migration.", evidence:"Strong - Tyler 2010: posterior capsule tightness corrected reduces impingement" },
      { name:"ESWT - Shockwave (calcific tendinopathy)", desc:"Focused ESWT x 2000 pulses x 3 sessions for calcific deposits. Best evidence for calcific rotator cuff tendinopathy.", evidence:"Strong - Gerdesmeyer 2003: ESWT superior to placebo for calcific RCT" },
      { name:"Dry Needling / Acupuncture (trigger points)", desc:"Target infraspinatus, supraspinatus, upper trapezius trigger points. 3-5 needles x 20 min. Reduces myofascial pain and improves ER ROM before exercise.", evidence:"Moderate - trigger point needling reduces shoulder pain in RCT" },
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Neuromuscular Control (Weeks 1-6)", color:"#00c97a", exercises:[
        ...
        { name:"Closed Chain Wall Press (Isometric ER)", sets:3, reps:15, hold:5, freq:"Daily", desc:"Elbow at 90deg, forearm against wall. Press outward isometrically. Closed chain - compresses glenohumeral joint. Safe stabiliser activation.", cues:"Press into wall. Feel deep rotators working. No shrug. Hold 5s.", evidence:"Strong - closed chain exercises safer than open chain in instability phase 1" },
      ...
      { phase:"Phase 2 - Dynamic Stability & Strength (Weeks 6-14)", color:"#ffb300", exercises:[
        { name:"External Rotation Strengthening (band - progressive)", sets:3, reps:20, hold:2, freq:"Daily", desc:"Band ER in neutral, then 45deg, then 90deg abduction. Infraspinatus + teres minor depresses humeral head, preventing anterior translation.", cues:"Elbow at side. Rotate outward slowly. Control return 3 seconds. No pain.", evidence:"Strong - Kuhn 2010 RCT: ER strengthening reduces instability events" },
        ...
        { name:"Sidelying ER in 90deg Abduction", sets:3, reps:15, hold:2, freq:"Daily", desc:"Sidelying, arm abducted to 90deg supported. Externally rotate against gravity/weight. Loads infraspinatus in functional position.", cues:"Keep arm at 90deg throughout. Rotate slowly. Feel posterior cuff contracting.", evidence:"Strong - posterior cuff functional loading for overhead instability" },
      ]},
      { phase:"Phase 3 - Sport/Function Specific (Weeks 14+)", color:"#ff4d6d", exercises:[
        { name:"Plyometric Ball Catch (wall throw)", sets:3, reps:15, hold:0, freq:"3x/wk", desc:"Throw small ball against wall and catch. Reactive shoulder muscle activation. Trains rapid co-contraction needed for sport.", cues:"Catch with elbow slightly bent. Absorb - don't let arm fly back. Fast reactive catch.", evidence:"Strong - plyometric shoulder loading for return to overhead sport" },
        { name:"90/90 ER Strengthening (throwing position)", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Arm abducted 90deg, elbow at 90deg. ER against band. The 90/90 position is where anterior instability is most at risk - must train here before return to throwing.", cues:"Shoulder at 90deg, elbow at 90deg. Rotate slowly. No pain. Build trust in position.", evidence:"Strong - position-specific strengthening for anterior instability clearance" },
      ]},
    ],
    treatment:[
  ...
  },
  {
    ...
    phases:[
      ...
      { phase:"Phase 2 - Frozen (Stiffness-Dominant, Weeks 9-26)", color:"#ffb300", exercises:[
        { name:"Capsular Stretching - ER (hand on door frame)", sets:3, reps:1, hold:30, freq:"3x/day", desc:"Stand in doorway. Elbow at 90deg, forearm on frame. Rotate body away from arm. Anterior capsule is tightest - primary stretch in phase 2.", cues:"Body turns away from arm. Feel anterior shoulder stretch. Hold 30s. Pain 4-5/10 acceptable.", evidence:"Strong - Favejee 2011: stretching = corticosteroid injection at 6 weeks" },
        { name:"Finger Walking (flexion and abduction)", sets:3, reps:10, hold:5, freq:"3x/day", desc:"Walk fingers up wall in flexion, then abduction. Mark progress. Gravity-assisted stretch at end of available range.", cues:"Walk to maximum height. Hold at top 5s. Mark your progress on wall with tape.", evidence:"Strong - standard frozen shoulder active ROM exercise" },
      ...
      { phase:"Phase 3 - Thawing (Recovery, Months 6-24)", color:"#ff4d6d", exercises:[
        { name:"Full ROM Strengthening - All planes", sets:3, reps:15, hold:0, freq:"3x/wk", desc:"As ROM returns, progressively strengthen through full available range. Include: flexion, abduction, ER/IR, scaption.", cues:"Work through as much range as available. Don't force. ROM is returning - strengthen what you have.", evidence:"Strong - progressive loading as capsule thaws naturally" },
        { name:"Overhead Press (progressive)", sets:3, reps:12, hold:0, freq:"3x/wk", desc:"Begin dumbbell press in available range. Progress toward full overhead. Final functional milestone in frozen shoulder recovery.", cues:"Start with arm below 90deg. Progress overhead as range permits. No cheating with trunk lean.", evidence:"Strong - return to overhead function as primary goal in thawing phase" },
      ]},
    ],
    treatment:[
...
const ELBOW_PROTOCOLS = [
  {
    ...
    phases:[
      { phase:"Phase 1 - Pain Control & Isometrics (Weeks 1-4)", color:"#00c97a", exercises:[
        ...
        { name:"Wrist Extensor Stretch (gentle)", sets:3, reps:1, hold:30, freq:"3x/day", desc:"Arm straight, palm down. Use other hand to bend wrist downward. Gentle sustained stretch to lateral epicondyle origin.", cues:"Straight elbow. Gentle bend. Feel pull at outer elbow. No sharp pain. 30s hold.", evidence:"Moderate - tissue extensibility maintenance in acute phase" },
      ...
      { phase:"Phase 2 - Heavy Slow Resistance (Weeks 4-12)", color:"#ffb300", exercises:[
        ...
        { name:"Wrist Extension - Dumbbell (HSR protocol)", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Forearm on table, palm down, holding dumbbell. Extend wrist upward (3s up, 3s down). Heavy slow resistance.", cues:"3 seconds up. Hold 2s at top. 3 seconds down. Heavy enough to limit to 15 reps.", evidence:"Strong - HSR superior to eccentric alone for tendinopathy at 6 months" },
        { name:"Forearm Pronation/Supination - Dumbbell", sets:3, reps:15, hold:1, freq:"3x/wk", desc:"Elbow at 90deg, hold dumbbell at one end (lever). Rotate forearm palm up to palm down.", cues:"Elbow fixed at side. Rotate slowly. The longer you hold the dumbbell end, the harder it is.", evidence:"Moderate - supinator loading reduces lateral elbow load" },
        { name:"Wrist Radial Deviation - Dumbbell", sets:3, reps:15, hold:1, freq:"3x/wk", desc:"Hold dumbbell at one end vertically. Raise thumb-side upward (radial deviation). ECRL and ECRB loading.", cues:"Wrist moves only - not elbow. Slow up and slow down.", evidence:"Moderate - full wrist extensor loading for lateral epicondylalgia" },
      ]},
      { phase:"Phase 3 - Function & Return to Sport/Work (Weeks 12+)", color:"#ff4d6d", exercises:[
        { name:"Grip Strengthening - Maximum (dynamometer)", sets:3, reps:10, hold:5, freq:"3x/wk", desc:"Maximum grip strength training. Return to full grip capacity. Key for return to racquet sports, manual work.", cues:"Maximum grip. Hold 5s. Track progress with dynamometer. Goal: equal to unaffected side.", evidence:"Strong - grip strength symmetry = return to sport readiness" },
        { name:"Sport-Specific Loading (racquet/tool simulation)", sets:3, reps:20, hold:0, freq:"3x/wk", desc:"Simulate sport or work movement under load. Gradual return to aggravating activity.", cues:"Start at 50% intensity. Add 10% per week. Pain <2/10 throughout.", evidence:"Strong - sport/work-specific loading for final stage return to function" },
      ]},
    ],
    treatment:[
      { name:"Deep Friction Massage (Cyriax)", desc:"Cross-friction massage directly to ECRB origin at lateral epicondyle. 5-10 min per session 3x/wk. Firm pressure perpendicular to tendon fibres.", evidence:"Moderate - Cyriax friction massage: short-term pain relief and tissue remodelling" },
      { name:"Lateral Elbow MWM (Mulligan)", desc:"Lateral glide of elbow while patient performs pain-free gripping. Immediate pain reduction. 3 sets x 10 reps per session.", evidence:"Strong - Vicenzino 2001: MWM immediate pain-free grip improvement" },
      { name:"ESWT - Shockwave Therapy", desc:"Radial ESWT x 2000 pulses x 3 sessions. Best for chronic lateral epicondylalgia >3 months.", evidence:"Strong - Rompe 2007 RCT: ESWT superior to corticosteroid at 12 months" },
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Load Reduction & Isometrics (Weeks 1-4)", color:"#00c97a", exercises:[
        ...
        { name:"Wrist Flexor Stretch", sets:3, reps:1, hold:30, freq:"3x/day", desc:"Arm straight, palm up. Other hand bends wrist into extension. Stretch to medial epicondyle origin.", cues:"Straight elbow. Gentle bend backward. Feel inner elbow stretch. 30s. No sharp pain.", evidence:"Moderate - tissue extensibility for common flexor origin" },
        { name:"Finger Flexor Tendon Gliding", sets:3, reps:10, hold:3, freq:"3x/day", desc:"Move fingers through full tendon glide sequence. Reduces adhesions, maintains tendon mobility in flexor mechanism.", cues:"Slow and deliberate through each position. Hold each 3s. No pain.", evidence:"Moderate - tendon gliding maintains flexor mechanism mobility" },
      ]},
      { phase:"Phase 2 - Progressive Loading (Weeks 4-12)", color:"#ffb300", exercises:[
        { name:"Wrist Flexion - Dumbbell (HSR)", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Forearm on table palm up. Curl wrist upward with dumbbell. 3s up, hold 2s, 3s down. Heavy slow resistance for flexor-pronator mass.", cues:"3 seconds up. 3 seconds down. Heavy enough to limit to 15 reps. Forearm supported throughout.", evidence:"Strong - HSR protocol adapted for medial epicondyle tendinopathy" },
        { name:"Forearm Pronation - Dumbbell (lever)", sets:3, reps:15, hold:1, freq:"3x/wk", desc:"Elbow at 90deg, hold dumbbell at one end. Rotate from supination to pronation. Loads pronator teres.", cues:"Elbow fixed. Rotate slowly to palm-down position. 3s each way.", evidence:"Strong - pronator teres loading essential for medial epicondylalgia" },
      ...
      { phase:"Phase 3 - Return to Sport/Activity (Weeks 12+)", color:"#ff4d6d", exercises:[
        { name:"Rotational Power Training (medicine ball)", sets:3, reps:12, hold:0, freq:"3x/wk", desc:"Rotational throw against wall with medicine ball. Loads medial elbow in valgus + flexion pattern.", cues:"Start light (1kg). Rotate through core and elbow. Pain <2/10. Build speed gradually.", evidence:"Strong - sport-specific rotational loading for return to throwing/golf" },
        { name:"Wrist Flexion - Max Load", sets:4, reps:8, hold:2, freq:"3x/wk", desc:"Maximum load wrist flexion dumbbell curls. 4 x 8 for maximum strength.", cues:"3s up, 3s down. Load heavy. Track with progressive weight.", evidence:"Strong - maximum strength goal for discharge readiness" },
      ]},
    ],
    treatment:[
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Nerve Protection & Neural Mobility (Weeks 1-6)", color:"#00c97a", exercises:[
        ...
        { name:"Elbow Flexion Avoidance Training", sets:1, reps:0, hold:0, freq:"Ongoing", desc:"Avoid prolonged elbow flexion beyond 90deg. Sleeping: pillow under arm. Phone: use speakerphone. Activity modification is primary intervention.", cues:"Avoid elbow bend beyond 90deg for sustained periods. Most important intervention in mild-moderate cubital tunnel.", evidence:"Strong - Svernlöv 2009: night splinting + activity modification = 90% improvement at 6 months" },
      ...
      { phase:"Phase 3 - Return to Full Function (Weeks 12+)", color:"#ff4d6d", exercises:[
        { name:"Progressive Grip Load - Dynamometer", sets:3, reps:10, hold:5, freq:"3x/wk", desc:"Measure grip strength with dynamometer. Progress to match unaffected side. Primary outcome measure.", cues:"Track progress. Goal: symmetrical grip strength. 10% per week increase maximum.", evidence:"Strong - grip strength symmetry primary outcome measure for cubital tunnel recovery" },
      ]},
    ],
    treatment:[
      { name:"Night Splint - Elbow Extension (30deg)", desc:"Splint keeping elbow at 30deg flexion during sleep. Most effective single intervention for mild-moderate cubital tunnel.", evidence:"Strong - Svernlöv 2009: night splinting = primary conservative treatment, 90% success mild-moderate" },
      ...
      { name:"Ergonomic Assessment", desc:"Assess workstation: keyboard height, armrest position, phone use. Sustained elbow flexion >90deg is primary provocateur.", evidence:"Strong - ergonomic modification reduces sustained nerve compression" },
  ...
  },
  {
    ...
    treatment:[
      ...
      { name:"Antibiotic Treatment (septic bursitis)", desc:"If septic: flucloxacillin 500mg QDS x 2 weeks. Refer to emergency if systemically unwell.", evidence:"Strong - antibiotic treatment mandatory for septic bursitis" },
    ]
  },
];
...
const HIP_PROTOCOLS = [
  {
    ...
    phases:[
      { phase:"Phase 1 - Pain Control & Activation (Weeks 1-4)", color:"#00c97a", exercises:[
        { name:"Supine Hip Abduction (sidelying)", sets:3, reps:15, hold:2, freq:"Daily", desc:"Sidelying, raise top leg 30deg. Hold 2s. Glute med activation without axial hip load. Safe in all hip OA stages.", cues:"Don't roll pelvis back. Toes slightly down. Hold at top.", evidence:"Strong - glute med activation without joint compression in hip OA" },
        ...
        { name:"Seated Hip Flexor Stretch", sets:3, reps:1, hold:30, freq:"3x/day", desc:"Sit at edge of chair. Let one leg drop behind. Tilt pelvis posteriorly. Stretch hip flexors - tight in hip OA.", cues:"Sit tall. Tilt pelvis back. Feel stretch at front of hip. 30s. No lumbar arch.", evidence:"Strong - hip flexor length restoration reduces anterior hip impingement in OA" },
      ]},
      { phase:"Phase 2 - Strength & Load (Weeks 4-10)", color:"#ffb300", exercises:[
        { name:"Mini Squat (0-45deg) - Wall Support", sets:3, reps:15, hold:2, freq:"Daily", desc:"Stand with back to wall. Squat to 45deg only. Functional quad + glute loading within pain-free range.", cues:"Weight even. Knees over 2nd toe. Stop at 45deg. Push through heels to return.", evidence:"Strong - OARSI recommended functional loading for hip OA" },
        ...
        { name:"Static Cycling", sets:1, reps:0, hold:0, freq:"20-30 min daily", desc:"Low resistance cycling. Seat high to limit hip flexion. Best aerobic exercise for hip OA.", cues:"Seat high enough so hip flexes only 70-80deg at bottom of stroke. No pain.", evidence:"Strong - NICE 2022: aerobic exercise first-line for hip OA" },
      ...
      { phase:"Phase 3 - Function & Long Term (Weeks 10+)", color:"#ff4d6d", exercises:[
        ...
        { name:"Leg Press - Single Leg (progressive)", sets:3, reps:12, hold:1, freq:"3x/wk", desc:"Single-leg press 0-60deg range. Progressive load. Best gym exercise for hip OA.", cues:"Push through heel. Don't lock knee. Control return 3 seconds.", evidence:"Strong - progressive overload for hip OA functional recovery" },
      ]},
    ],
    treatment:[
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Load Management & Tendon Protection (Weeks 1-6)", color:"#00c97a", exercises:[
        ...
        { name:"Clam Exercise (glute med isolation)", sets:3, reps:20, hold:2, freq:"Daily", desc:"Sidelying, hips at 30deg flexion (not more). Band above knees. Open top knee upward. Glute med isolation without compressive tendon load.", cues:"Hips at 30deg only - more flexion compresses tendon. Foot stays grounded. Slow open.", evidence:"Strong - glute med activation without tendon compression in early GTPS" },
      ...
      { phase:"Phase 2 - Progressive Tendon Loading (Weeks 6-12)", color:"#ffb300", exercises:[
        { name:"Single-Leg Bridge (eccentric control)", sets:3, reps:12, hold:2, freq:"3x/wk", desc:"Bridge up bilateral. Lower one leg. Hold single-leg bridge 2s. Progressive unilateral glute loading.", cues:"Keep pelvis level during single-leg hold. Don't let hip drop. Build to 12 reps.", evidence:"Strong - progressive unilateral glute loading for gluteal tendinopathy" },
        { name:"Side-Lying Hip Abduction - Weighted", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Sidelying, ankle weight. Abduct top leg to 30deg. Hold 2s. Slow lower. Progressive glute med isotonic loading.", cues:"Toes slightly down. 30deg abduction only. Hold at top. 3s lower. Don't roll pelvis.", evidence:"Strong - isotonic glute med loading for GTPS" },
        { name:"Standing Hip Abduction - Band (heavy)", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Heavy resistance band. Stand on one leg. Abduct other against band. Functional WB glute med loading.", cues:"Stand tall. Pelvis level. Abduct slowly. Hold 2s. No hip hike or lean.", evidence:"Strong - functional glute med WB loading for GTPS" },
        { name:"Wall Squat - Bilateral (0-60deg)", sets:3, reps:15, hold:2, freq:"Daily", desc:"Back to wall. Squat to 60deg. Bilateral - avoids hip adduction. Glute + quad functional loading.", cues:"Feet shoulder width. Knees tracking over toes. Keep weight even. No single-leg bias.", evidence:"Moderate - bilateral squat for GTPS functional strength" },
      ]},
      { phase:"Phase 3 - Energy Storage & Function (Weeks 12+)", color:"#ff4d6d", exercises:[
        { name:"Single-Leg Squat (controlled)", sets:3, reps:12, hold:2, freq:"3x/wk", desc:"Single-leg squat to 45deg. Knee over 2nd toe. Pelvis level. Primary functional test AND exercise for GTPS.", cues:"Slow 3s down. Hold 2s. Push through heel up. Knee straight over 2nd toe. No valgus.", evidence:"Strong - functional single-leg loading for return to activity in GTPS" },
        { name:"Step-Down Eccentric (lateral)", sets:3, reps:10, hold:0, freq:"3x/wk", desc:"Stand on step. Lower non-affected foot slowly (4s). Eccentric glute control. Most demanding functional exercise.", cues:"Count 4 seconds down. Knee tracks over 2nd toe. Pelvis level throughout.", evidence:"Strong - eccentric glute loading for return to sport/stairs" },
        { name:"Running / Walking Programme - Graded", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Graded return to running. Start: walk-jog intervals. Progress 10% per week.", cues:"No camber running. Flat surface. Increase time before speed.", evidence:"Strong - graded running load for return to sport in GTPS" },
      ]},
    ],
    treatment:[
      ...
      { name:"ESWT - Shockwave", desc:"Radial ESWT x 2000 pulses x 3 sessions. Apply to point of maximum tenderness over greater trochanter.", evidence:"Strong - Rompe 2009: ESWT superior to home training and corticosteroid at 15 months" },
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Neural & Muscle Release (Weeks 1-4)", color:"#00c97a", exercises:[
        { name:"Piriformis Stretch - Figure 4 (supine)", sets:3, reps:1, hold:30, freq:"3x/day", desc:"Lie on back. Cross affected ankle over opposite knee. Pull both knees toward chest. Deep buttock stretch.", cues:"Keep ankle flexed (dorsiflexed). Pull gently. Feel deep buttock stretch. 30s. No pins and needles.", evidence:"Strong - primary stretch for piriformis shortening and sciatic nerve irritation" },
      ...
      { phase:"Phase 2 - Strength & Stability (Weeks 4-10)", color:"#ffb300", exercises:[
        { name:"Clam Exercise - Glute Med", sets:3, reps:20, hold:2, freq:"Daily", desc:"Sidelying, hips at 60deg flexion, band above knees. Open top knee upward. Piriformis overactivates when glute med is weak.", cues:"Hips at 60deg. Foot stays grounded. Open slowly. Feel outer glute, not deep.", evidence:"Strong - glute med strengthening reduces piriformis compensatory overactivation" },
        ...
        { name:"Standing Hip Abduction - Weighted", sets:3, reps:15, hold:2, freq:"3x/wk", desc:"Hold wall for balance. Abduct leg against ankle weight. Glute med strengthening in weight-bearing.", cues:"Stand tall. Don't lean. Abduct 30deg. Hold 2s. Level pelvis throughout.", evidence:"Strong - WB glute med loading for piriformis syndrome management" },
      ]},
      { phase:"Phase 3 - Function & Return to Activity (Weeks 10+)", color:"#ff4d6d", exercises:[
        { name:"Single-Leg Squat - Controlled", sets:3, reps:12, hold:2, freq:"3x/wk", desc:"Single-leg squat to 45deg. Tests glute med and max function - if piriformis syndrome resolved, should be pain-free.", cues:"Control knee. Level pelvis. No deep buttock pain. Progress depth gradually.", evidence:"Strong - functional return to activity test for piriformis syndrome" },
        { name:"Running Cadence + Hip Extension Training", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Graded return to running with focus on hip extension at push-off and gluteal activation.", cues:"Run tall. Drive hip back. Slight forward lean. Monitor for deep buttock pain.", evidence:"Strong - biomechanical running correction reduces piriformis overload" },
      ]},
    ],
    treatment:[
      { name:"Dry Needling - Piriformis Trigger Points", desc:"Trigger point dry needling to piriformis muscle belly. Patient prone. 3-5 needles x 20 min.", evidence:"Strong - Fishman 2002: dry needling superior to injection and stretching alone" },
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Load Protection & Isometrics (Weeks 1-6)", color:"#00c97a", exercises:[
        { name:"Isometric Hamstring Contraction (prone)", sets:5, reps:1, hold:45, freq:"Daily", desc:"Lie face down. Bend knee to 90deg. Partner resists downward. Maximum isometric contraction 45s. Analgesic effect.", cues:"Maximum effort push - no movement. 45 seconds. Pain 0-4/10 acceptable.", evidence:"Strong - isometric loading for proximal hamstring tendinopathy pain relief" },
        { name:"Sitting Posture Management", sets:1, reps:0, hold:0, freq:"Ongoing", desc:"CRITICAL: Avoid prolonged sitting on hard surfaces. Use cushion. Keep hip flexion <70deg when sitting.", cues:"Sit on soft surface. Lean forward slightly. No crossing legs. Stand every 30 min.", evidence:"Strong - compressive load avoidance is primary intervention in proximal hamstring tendinopathy" },
      ...
      { phase:"Phase 2 - Heavy Slow Resistance (Weeks 6-14)", color:"#ffb300", exercises:[
        { name:"Deadlift - Hip Hinge (progressive load)", sets:3, reps:8, hold:1, freq:"3x/wk", desc:"Hip hinge deadlift. Start bodyweight, progress to barbell. 3s down, 3s up. Heavy slow resistance. Best exercise for PHT.", cues:"Push hips back. Spine neutral. Hamstring stretch at bottom. Drive hips forward. SLOW is key.", evidence:"Strong - HSR deadlift primary evidence-based exercise for proximal hamstring tendinopathy" },
        { name:"Nordic Hamstring Curl (eccentric)", sets:3, reps:6, hold:0, freq:"3x/wk", desc:"Kneel, feet fixed. Lower body forward slowly (3-5s). Eccentric hamstring loading.", cues:"Lower as slowly as possible. 5 seconds down. Catch yourself at bottom.", evidence:"Strong - Petersen 2011: Nordic curls reduce hamstring injury and strengthen proximal tendon" },
        { name:"Romanian Deadlift - Single Leg", sets:3, reps:10, hold:1, freq:"3x/wk", desc:"Single-leg RDL with dumbbell. Balance + hamstring + glute loading simultaneously.", cues:"Hip hinge on one leg. Spine neutral. Feel hamstring tension. Control balance.", evidence:"Strong - unilateral loading for PHT sport-specific strength" },
      ]},
      { phase:"Phase 3 - Energy Storage & Return to Sport (Weeks 14+)", color:"#ff4d6d", exercises:[
        { name:"Sprint Mechanics - Graded Return", sets:4, reps:6, hold:0, freq:"2x/wk", desc:"Graded sprint: jog -> stride -> 75% -> 90% -> 100%. Monitor PHT pain 24h post-session.", cues:"Build speed over 4 weeks. Track pain 24h post. No sprinting if pain >2/10 next day.", evidence:"Strong - graded sprint exposure for return to running sport with PHT" },
        { name:"Barbell Deadlift - Max Strength", sets:4, reps:5, hold:1, freq:"2x/wk", desc:"Heavy barbell deadlift. 4 x 5 for maximum posterior chain strength. Goal: 1.5x bodyweight.", cues:"Maximum load with perfect form. Track 1RM progress. Goal = 1.5x BW.", evidence:"Strong - maximum strength benchmark for PHT return to sprint sport clearance" },
      ]},
    ],
    treatment:[
      { name:"ESWT - Shockwave (ischial tuberosity)", desc:"Focused ESWT x 2000 pulses directly over ischial tuberosity. 3 sessions x weekly. Best for chronic PHT >3 months.", evidence:"Strong - Cacchio 2011 RCT: ESWT superior to exercise alone for chronic PHT" },
  ...
  },
  {
    ...
    phases:[
      { phase:"Phase 1 - Symptom Control & Flexibility (Weeks 1-4)", color:"#00c97a", exercises:[
        { name:"Iliopsoas Stretch - Kneeling Lunge", sets:3, reps:1, hold:40, freq:"3x/day", desc:"Kneel on affected knee. Step other foot forward. Push hips forward. Posterior pelvic tilt during stretch.", cues:"Tuck pelvis under (posterior tilt). Feel stretch at front of hip. Hold 40s. No arch in back.", evidence:"Strong - iliopsoas lengthening primary intervention for iliopsoas bursitis and snapping hip" },
      ...
      { phase:"Phase 2 - Strengthening & Movement Retraining (Weeks 4-10)", color:"#ffb300", exercises:[
        ...
        { name:"Glute Med Strengthening - Clam + Band Walk", sets:3, reps:20, hold:2, freq:"Daily", desc:"Glute med weakness drives anterior pelvic tilt and iliopsoas overload.", cues:"Clam: hips 60deg. Band walk: stay low. Pelvis level. Don't lean.", evidence:"Strong - glute med strengthening reduces anterior pelvic tilt driving snapping hip" },
      ]},
      { phase:"Phase 3 - Return to Full Activity (Weeks 10+)", color:"#ff4d6d", exercises:[
        { name:"Full Hip Flexion Strengthening - Resistance", sets:3, reps:12, hold:1, freq:"3x/wk", desc:"Cable or band hip flexion against resistance. Full controlled arc. Tests iliopsoas under load without snapping.", cues:"Controlled lift to 90deg. Slow return. No snapping. Progress resistance gradually.", evidence:"Strong - full-range loaded hip flexion for iliopsoas return to function" },
        { name:"Running - Graded Return", sets:1, reps:0, hold:0, freq:"3x/wk", desc:"Graded return to running. Monitor for snapping during running gait.", cues:"No snapping during jog = safe to progress. Pain or snap: reduce pace and distance.", evidence:"Strong - graded running return for iliopsoas bursitis / snapping hip" },
      ]},
    ],
    treatment:[
          ...
          {programme.map((ex,i)=>(
            <div key={ex.id} style={{background:"#f5f0fb",border:"1px solid #d8cce8",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:9}}>
                ...
                <button onClick={()=>removeEx(ex.id)} style={{background:"none",border:"1px solid #d8cce8",borderRadius:6,color:"#7e6a9a",cursor:"pointer",fontSize:"0.65rem",padding:"2px 7px"}}>x</button>
              </div>
...
// The SVG viewBox is 200x420 (front left, back right, side by side)
                    ...
                    <div key={pin.id} onClick={() => setSelected(pin.id)}
                      ...
                      <button onClick={e => { e.stopPropagation(); removePin(pin.id); }}
                        style={{ background:"none", border:"none", color:C.muted,
                          cursor:"pointer", fontSize:"0.72rem", padding:"0 3px", lineHeight:1 }}>x</button>
                    </div>
                  );
                })}
              </div>
    ...
    switch(activeTab) {
      case "manual": return (
        <div style={{display:"grid",gap:8}}>
          ...
          <div className="pm-grid-2">
            <div><label style={lbl}>Sets / Reps or Duration</label><input value={form.dosage} onChange={e=>fset("dosage",e.target.value)} placeholder="e.g. 3x30s, 60 oscillations" style={inp}/></div>
...
function TreatmentSessionLogModule({ data, set }) {
  ...
  const autoHEP = useMemo(() => {
    ...
    return hep.map(ex => `${ex.name} - ${ex.customSets}x${ex.customReps}, hold ${ex.customHold}s, ${ex.customFreq}`).join("; ");
  }, [data?.hep_programme]);
