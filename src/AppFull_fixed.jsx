...
function PostureAnalysisModule(){
  ...
  // Refs that mirror state — used inside the RAF loop to avoid stale closures
  const patientHeightCmRef=useRef(patientHeightCm);
  const processLandmarksRef=useRef(null);

  useEffect(()=>{viewRef.current=view;},[view]);
  useEffect(()=>{patientHeightCmRef.current=patientHeightCm;},[patientHeightCm]);

  // ── Load MediaPipe ──────────────────────────────────────────────────────────
  useEffect(()=>{
    let cancelled=false;
    const tryInit = async (complexity) => {
    ...
    (async()=>{
      try{
        // First attempt: full quality (complexity 1)
        await tryInit(1);
      }catch(e){
        if(cancelled) return;
        try{
          // Retry: lower complexity — works on older/low-end devices
          await tryInit(0);
        }catch(e2){
  ...
  const processLandmarks=useCallback((lm,v,imgH)=>{
    const calib = computeCalibration(lm, patientHeightCmRef.current, imgH||videoSizeRef.current?.h||480);
  ...
  useEffect(()=>{ processLandmarksRef.current=processLandmarks; },[processLandmarks]);
  ...
  async function startCamera(facing="environment"){
    // Allow start even if AI failed — camera/video works independently of MediaPipe
    if(mpStatus==="loading"){setError("AI model still loading — please wait a moment");return;}
    ...
    try{
      ...
      const handler=results=>{
        if(results.poseLandmarks?.length>0){
          ...
          // Use ref so we always call the latest processLandmarks (avoids stale closure)
          if(processLandmarksRef.current) processLandmarksRef.current(lm,viewRef.current,H);
        }
        if(overlayRef.current&&videoRef.current){
          ...
          if(W>0&&H>0){
            ...
            // Use patientHeightCmRef so the overlay always reflects the current height
            const liveCalib = results.poseLandmarks
              ? computeCalibration(results.poseLandmarks, patientHeightCmRef.current, H) : null;
      ...
      // Only attach MediaPipe if it loaded successfully — camera video works regardless
      if(poseRef.current){
        poseRef.current.onResults(handler);
      }
      ...
      const loop=async()=>{
        ...
        if(poseRef.current && videoRef.current?.readyState>=2 && now-lastSend>=INTERVAL){
          lastSend=now;
          ...
          <div>
            {!camReady?(
              <div style={{padding: isWide?"20px":"16px",display:"flex",flexDirection:"column",gap:10}}>
                ...
                {/* AI status pill */}
                <div style={{padding:"8px 12px",borderRadius:9,fontSize:"0.72rem",fontWeight:700,
                  background: mpStatus==="ready"?"rgba(0,201,122,0.08)": mpStatus==="error"?"rgba(220,38,38,0.08)":"rgba(255,179,0,0.08)",
                  color: mpStatus==="ready"?PC.green: mpStatus==="error"?PC.red:PC.yellow,
                  border:`1px solid ${mpStatus==="ready"?PC.green: mpStatus==="error"?PC.red:PC.yellow}30`,
                  display:"flex",alignItems:"center",gap:8}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:"currentColor",display:"inline-block",
                    animation:mpStatus==="loading"?"pcPulse 1.3s infinite":"none"}}/>
                  {mpStatus==="ready"?"✓ AI Ready — tap a camera below"
                    :mpStatus==="error"?"⚠ AI failed to load — camera still available (no skeleton overlay)"
                    :"⏳ Loading AI model… (first load takes ~10s)"}
                </div>
                {/* Camera start buttons — blocked ONLY while actively loading, not on error */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {[["environment","▣ Back Camera"],["user","⎇ Front Camera"]].map(([f,label])=>(
                    <button key={f} onClick={()=>startCamera(f)} disabled={mpStatus==="loading"}
                      style={{padding: isWide?"16px":"13px",borderRadius:12,border:`1px solid ${PC.border}`,background:PC.surface,
                        color:mpStatus!=="loading"?PC.text:PC.muted,fontWeight:700,
                        fontSize: isWide?"0.85rem":"0.78rem",cursor:mpStatus!=="loading"?"pointer":"not-allowed"}}>
                      {label}
                    </button>
                  ))}
                </div>
