...
| Your call: the old step-by-step "Screening Workflow" removed -- its step bar and pages (Demographics, Body Regions, Subjective, AI, Chart/Palpation, Objective), the standalone ROM/MMT/Special Tests/Neuro/Gait/... pages only it opened, the old config-driven "stream" engine, and 20 files only they used. Kept: Treatment / Exercise Prescription / Home Protocol (patient profile's "Edit Program" and Learn open it) and Posture Analysis. A saved or browser-history link to a removed page now opens Home. `PhysioNeuro.jsx` is now unused but left in place while another session edits it -- delete it once that work is committed | `625947a`, `7bf7f23` |
| Leftover code of the old workflow removed from the shared files: the old Subjective screen and its analysis panels (`SubjectiveObjective.jsx` 6,335 -> ~1,100 lines; it now only holds `runEngineV6` for the AI test harness, `REG_MOD_S`, `SmallClinicalImg`, `FunctionalScreenHub`), old Gait/Observation/Outcome/Session pages in `ClinicalModules.jsx` (6,118 -> ~4,300), the old stream widgets, 11 files only they used, and the 3 `api/extract*NoteVariables.js` AI endpoints only the old Subjective called. 27 checks that only tested removed code deleted; checks for live code kept (1,014 pass, 0 skipped) | this commit, "Remove the old workflow's leftover code" |

Still to do from this cleanup:
- `PhysioNeuro.jsx` (old ROM/MMT/Neuro pages, ~2,600 lines) is unused -- delete once the other session's edits in it are committed.
- `lumbar/cervical/thoracicVariableExtractor.js` are now only used by the checks for the live reasoning engines. Rewrite those checks to build patients with the new Ortho adapters (`orthoLumbarReasoning.js` etc.), then delete the three files.
- The browser tests in `e2e/` (run on pull requests / by hand) still walk the old workflow (New Patient -> Subjective -> Suggest objective -> ROM/MMT tiles) and need rewriting for today's screens.
