...
| Your call: the whole AI Assistant chat removed (`AIAssistant.jsx`, the `api/chat.js` server endpoint, its screen entry). No button led to it any more, and the endpoint was marked "not shipping", so nothing visible changes; it also closes an unused paid AI endpoint. The AI-assisted assessment (speak/type the Subjective, `api/parse.js`) is separate and stays | this commit, "Remove the AI Assistant chat" |
