...
export default async function handler(req, res) {
  ...
  // Same auth + rate-limit gate as api/parse.js (api/_lib/rateLimit.js) --
  // this endpoint had the identical zero-auth gap, flagged but not fixed
  // when /api/parse was done. Closing it now.
  const userId = await authenticateAndRateLimit(req, res, 'extractThoracic');
  if (!userId) return;
