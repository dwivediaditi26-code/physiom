...
Return this exact JSON shape (null for anything not mentioned, empty array [] for arrays):
{
  ...
  "region": one of ["Lumbar / SI","Cervical spine","Thoracic spine","Shoulder (L)","Shoulder (R)","Knee (L)","Knee (R)","Hip / Groin","Ankle / Foot","Elbow/Wrist/Hand"] or null. Map common lay terms to the right enum, and always pick the single best-fit region rather than returning null when the patient clearly names a body area: a generic "back"/"backache"/"back pain" with no level specified => "Lumbar / SI" (low back is by far the most common; only use "Thoracic spine" if they specify upper/mid back or between the shoulder blades, or "Cervical spine" if they mean the neck); "lower back"/"lumbar"/"SI"/"sacroiliac"/"buttock" => "Lumbar / SI"; "upper back"/"mid back"/"between shoulder blades" => "Thoracic spine"; "neck" => "Cervical spine"; "wrist"/"hand"/"elbow"/"forearm" => "Elbow/Wrist/Hand"; "ankle"/"foot"/"heel" => "Ankle / Foot"; "hip"/"groin" => "Hip / Groin". Only return null if no body area is identifiable at all.
  "additionalRegions": array (0-2) of ANY OTHER regions from the same list above that the patient ALSO complains about in the same narrative, when more than one distinct body area is described (e.g. neck AND knee pain in one intake). Put the PRIMARY/most prominent complaint in "region" and every other distinct region here. Use the exact enum strings. Empty [] if only one region is described. Do NOT list the same region twice, and do not include the one already in "region".
  "laterality": "Left"|"Right"|"Bilateral"|null,
  "conditionCategory": one of ["arthritis","softTissue","spine","sportsOveruse","postSurgicalFollowUp","painFunctional","other"] -- classify the presenting problem into exactly one bucket, using ONLY what the narrative actually supports (this drives which objective tests get suggested downstream, so treat it with the same evidence standard as every other field):
    - "arthritis": chronic joint pain / degenerative change (e.g. osteoarthritis, long-standing stiffness with no single injury)
    - "softTissue": a sprain, strain, or tendinopathy -- an acute/subacute soft-tissue injury, not a degenerative or spine presentation
    - "spine": neck or back pain, or any report of radiculopathy-type symptoms (pain/numbness/tingling radiating into an arm or leg)
    - "sportsOveruse": explicitly tied to sport or a repetitive-strain activity
    - "postSurgicalFollowUp": patient describes being post-operative / recovering after a named surgery, now at the outpatient follow-up stage
    - "painFunctional": pain or a functional limitation with no clear structural diagnosis named or implied
    - "other": doesn't clearly fit any of the above, or there is genuinely not enough information to classify
    Default to "other" whenever the narrative doesn't clearly support one of the more specific buckets -- never pick a specific category (e.g. "spine") from a vague complaint just because it's statistically common for that presentation. Always include this field with one of the 7 values (never null) and include it in _confidence/_sourceQuotes like any other filled field.
  ...
  "flags": array of red-flag strings the narrative actually mentions, or []. A red flag is any feature suggesting serious pathology needing medical screening/referral, NOT ordinary musculoskeletal pain. Actively screen the narrative for, and include when present: unexplained weight loss; night pain unrelieved by position change / constant unremitting pain; fever, chills or night sweats; history of cancer/malignancy; saddle anaesthesia or bladder/bowel dysfunction (cauda equina); progressive or bilateral neurological deficit; significant trauma, or minor trauma in someone with osteoporosis/steroid use (fracture risk); first severe episode age <20 or >55; recent significant infection; widespread or worsening weakness; dizziness/drop attacks/visual or speech disturbance with neck symptoms (VBI); throbbing abdominal/back pain in an older patient (AAA); IV drug use or immunosuppression. Quote the patient's own wording where possible (e.g. "unexplained 6 kg weight loss over 2 months", "pain wakes him every night, no position helps"). Only include a flag if the narrative genuinely supports it -- an explicit denial ("no weight loss, no night pain") is NOT a flag. [] if none present.
