...
test("hip journey: pick region, run analysis, click every objective-assessment tile (no crash)", async ({ page }) => {
  ...
  await page.getByText("Lower limb", { exact: false }).first().click().catch(() => {});
  await page.getByText("Hip / Groin", { exact: true }).first().click().catch(() => {});
  await page.getByRole("button", { name: "Left", exact: true }).first().click().catch(() => {});
  // close the picker again
  if (await editBtn.count()) await editBtn.first().click().catch(() => {});
